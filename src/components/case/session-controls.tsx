...
export function SessionControls(){
 ...
 async function logout(){
  ...
  setError('');try{const r=await fetch('/api/cases/access/request',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'logout'})});if(!r.ok)throw new Error();window.location.assign('/login');}catch{setError('היציאה לא הושלמה. אפשר לנסות שוב.');}
 }
