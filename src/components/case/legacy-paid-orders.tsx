...
export function LegacyPaidOrders({receipts,publicId}:{receipts:Awaited<ReturnType<typeof legacyCustomerReceipts>>;publicId:string}){
 return <>{receipts.map(receipt=><article key={receipt.id} data-legacy-order-id={receipt.id}>
  ...
  {receipt.period_state==='missing'?<><p>חסרה תקופה מזוהה במסמכים. אפשר לפתוח את המקור בבקשות התיק ולהעתיק את סוג המסמך ואת התאריכים שמופיעים בו. אין צורך לשלם שוב.</p>
    <a className="button button--primary" href={`/case/${publicId}/thread`}>זיהוי המסמך והתקופה בבקשות התיק</a></>
