...
function SupportForm({publicId,threadId}:{publicId:string;threadId?:string}){
 const router=useRouter(),retry=useRef<{id:string;message:string}|null>(null);
 const [message,setMessage]=useState(''),[busy,setBusy]=useState(false),[notice,setNotice]=useState('');
 async function send(){if(busy)return;setBusy(true);if(retry.current?.message!==message)retry.current={id:crypto.randomUUID(),message};try{const r=await fetch(`/api/cases/${publicId}/requests`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...retry.current,action:threadId?'support_reply':'support_open',...(threadId?{threadId}:{})})});if(!r.ok)throw new Error();setMessage('');setNotice('ההודעה נשמרה בשרשור התמיכה.');router.refresh();}catch{setNotice('לא התקבל אישור שמירה. הטקסט נשאר כאן ואפשר לנסות שוב.');}finally{setBusy(false);}}
 return <div><label>{threadId?'הוספת תשובה לפנייה':'פרטי הפנייה לתמיכה'}<textarea value={message} onChange={e=>setMessage(e.target.value)} maxLength={2000} rows={4} disabled={busy}/></label><button type="button" disabled={busy||message.trim().length<4} onClick={send}>{busy?'שומרים…':'שליחת הודעה'}</button><p role="status">{notice}</p></div>;
}
