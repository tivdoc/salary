...
function SupportForm({publicId,threadId}:{publicId:string;threadId?:string}){
 ...
 return <div><label htmlFor={fieldId}>{threadId?'הוספת תשובה לפנייה':'פרטי הפנייה לתמיכה'}</label><textarea id={fieldId} value={draft.message} onChange={e=>edit(e.target.value)} maxLength={2000} rows={4} disabled={!ready||busy||draft.pendingId!==null}/><button type="button" disabled={!ready||busy||draft.message.trim().length<4} onClick={send}>{busy?'שומרים…':'שליחת הודעה'}</button><p role="status">{notice}</p></div>;
}
