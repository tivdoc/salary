...
it('links a typed June declaration to its exact source without describing it as legal approval or an optional accuracy hint',()=>{
 const html=render({...row,code:'minimum_wage_june2026:'+'a'.repeat(64),field_crop:'minimum_wage'});
 expect(html).toContain('requests?source='+row.id);expect(html).toContain('כהצהרתך');expect(html).toContain('אינה אישור משפטי');
 expect(html).not.toContain('השדה בתלוש: minimum_wage');expect(html).not.toContain('לא מעכב את הבדיקה');
});
