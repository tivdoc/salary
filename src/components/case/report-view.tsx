...
function money(
  value: Readonly<{ currency: "ILS"; minor_units: number }>,
): string {
  return formatPrice({
    amount: (value.minor_units / 100).toFixed(2),
  ...
  return (
    <li className={`report-topic report-topic--${topic.gate}`}>
      ...
      {topic.gate === "checked" ? (
        <>
          {topic.severity_class === null ? null : (
            <p className="report-topic__severity">
              {SEVERITY_TEXT[topic.severity_class]}
            </p>
          )}
          <p className="report-topic__direction">
            {DIRECTION_HE[topic.direction]}
            {/* D-6.3: a number appears only where the display allows one. */}
            {permission.showsNumber && topic.amount
              ? ` · ${money(topic.amount)}`
              : null}
            {permission.showsNumber && topic.range
              ? ` · ${money(topic.range.low)}–${money(topic.range.high)}`
              : null}
          </p>
          {topic.assumptions.map((assumption) => (
            <p className="report-topic__assumption" key={assumption.slot}>
              {assumption.statement}
            </p>
          ))}
          {topic.retroactive_update ? (
            <p className="report-topic__retro">
              עודכן רטרואקטיבית ב־{topic.retroactive_update.known_at} (
              {topic.retroactive_update.source})
            </p>
          ) : null}
          {topic.missing_facts.length > 0 ? (
            <p className="report-topic__missing">
              מה שיעלה את הוודאות: {topic.missing_facts.join(", ")}
            </p>
          ) : null}
        </>
      ) : null}
      ...
      {topic.gate === "refused" ? (
        <>
          ...
          {mappingFor(topic.not_checked.code)?.question ? (
            <p className="report-topic__request">
              המידע הדרוש להמשך מופיע בהודעות בתיק.
            </p>
          ) : null}
        </>
      ) : null}
      ...
      {topic.branches_examined.length > 0 ? (
        <p className="report-topic__branches">
          נבחנו: {topic.branches_examined.join(" · ")}
        </p>
      ) : null}
    </li>
  );
  ...
  return (
    <div className="report-view">
      <div className="received-card">
        <Heading>מה נבדק בתיק</Heading>
        <p>
          חודש הבדיקה: {projection.check_period_month}
          {projection.months_covered.length > 1
            ? ` · כיסוי: ${projection.months_covered.join(", ")}`
            : null}
        </p>
        {/* The reader is told what state the whole report is in before any topic. */}
        {checked.length === 0 ? (
          <p className="report-view__all-awaiting">
            עדיין לא נבדק אף נושא. אין תוצאת בדיקה; פרטי הכיסוי והמידע החסר
            מופיעים בהמשך.
          </p>
        ) : (
          <p>
            {findings.length > 0
              ? `נמצאו ${findings.length} נקודות לבדיקה.`
              : "לא נמצאו פערים בנושאים שנבדקו."}
          </p>
        )}
      </div>
      ...
      {findings.length > 0 ? (
        <div className="received-card">
          ...
          <p>
            אפשר לפנות למעסיק עם הנקודות שנמצאו. הדוח המלא כולל נוסח מוכן להעתקה
            לכל ממצא וצעד הבא.
          </p>
          {/* D-5.1 before the CTA, always. */}
          <p className="report-view__second-product">
            {offer.second_product_sentence}
          </p>
        </div>
      ) : null}
