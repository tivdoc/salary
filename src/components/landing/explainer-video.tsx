...
export function ExplainerVideo({
  poster = "/media/tivdoc-explainer-poster.png",
}: {
  poster?: string;
}) {
  ...
  return (
    <>
      ...
      >
        <source
          src="/media/tivdoc-explainer.mp4"
          type="video/mp4"
          onError={() => setFailed(true)}
        />
        <track
      ...
      <a
        className="video-text-link"
        href="#explainer-transcript"
        onClick={() => {
      ...
      >
        להסבר הכתוב של הסרטון ←
      </a>
      {failed ? (
        <p className="explainer-video-error" role="status">
          הסרטון לא נטען.{" "}
          <a
            href="#explainer-transcript"
            onClick={() => {
