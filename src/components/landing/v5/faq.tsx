  ...
  const items = [
    {
      ...
      answer:
        // S4 (3.1/3.2): the boundary sentence, in the same words the case link
        // and the report-ready message use. The FAQ is where a reader decides
        // what this product claims to be, and it should say the same thing the
        // messages say rather than leaving them to say it alone.
        "סכום מוצג רק כשכל העובדות שהנושא דורש הגיעו ממסמך ואין סתירה ביניהן. כשעובדה נדרשת הגיעה מתשובה שלך — מוצג טווח; כשעובדה חסרה או שני מקורות סותרים — מוצג כיוון בלבד, בלי מספר. זה כלל מכני, לא שיקול דעת, והבדיקה תמיד אומרת מה חסר כדי להעלות את הוודאות. Tivdoc בודקת, לא קובעת: הדוח מציג נקודות לבדיקה ורמת ודאות, לא קביעה משפטית.",
    },
    {
