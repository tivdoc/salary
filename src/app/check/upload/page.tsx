...
export default async function UploadPage({searchParams}: {searchParams?: Promise<{requestId?: string;replaceVersionId?: string}>}) {
  ...
  const requested = params?.replaceVersionId ? undefined : params?.requestId;
  ...
  return (
    <>
      ...
      <DocumentReview key={`${caseId}:${selectedRequestId ?? ""}:${params?.replaceVersionId ?? ""}`} initial={snapshot} initialRequestId={selectedRequestId} replacementVersionId={params?.replaceVersionId} />
    </>
  );
}
