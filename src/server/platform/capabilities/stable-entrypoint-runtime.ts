...
export function validateStableEntrypointCapabilityRequirements(): readonly string[] {
  const issues: string[] = [];
  if (inventory.entries.length !== 110) issues.push("CAPABILITY_ENTRYPOINT_DENOMINATOR_CHANGED");
  ...
  if (inventory.entries.filter((entry) => entry.product_stable).length !== 99) issues.push("CAPABILITY_PRODUCT_STABLE_DENOMINATOR_CHANGED");
  if (STABLE_PRODUCT_DISPATCHER_ROOTS.length !== 42) issues.push("CAPABILITY_PRODUCT_DISPATCHER_DENOMINATOR_CHANGED");
