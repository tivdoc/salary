...
export const ROUTE_SPLIT: readonly RouteAssignment[] = Object.freeze([
  ...
  { entrypoint_id: "CEP-003", route_file: "src/app/check/upload/page.tsx", stable_entry: "/check/upload", half: "product", reason: `${MAIN}: the upload page.`, probes: [{ method: "GET", path: "/check/upload", expected: "200" }] },
  { entrypoint_id: "CEP-004", route_file: "src/app/check/payment/page.tsx", stable_entry: "/check/payment", half: "product", reason: `${MAIN}: the payment hand-off page.`, probes: [{ method: "GET", path: "/check/payment", expected: "200" }] },
  { entrypoint_id: "CEP-005", route_file: "src/app/check/received/page.tsx", stable_entry: "/check/received", half: "product", reason: `${MAIN}: the payment confirmation page.`, probes: [{ method: "GET", path: "/check/received", expected: "200" }] },
  ...
  { entrypoint_id: "CEP-096", route_file: "src/app/case/[token]/page.tsx", stable_entry: "/case/[token]", half: "product", reason: "UX Run 1 / D-1.2: the code challenge the sent link opens, and the case view for a verified identity session; it reaches no legal computation.", probes: [{ method: "GET", path: "/case/probe", expected: "200" }] },
  { entrypoint_id: "CEP-097", route_file: "src/app/login/page.tsx", stable_entry: "/login", half: "product", reason: "UX Run 1 / D-1.4: phone or email, then a code, then the identity's cases; login and recovery in one route.", probes: [{ method: "GET", path: "/login", expected: "200" }] },
  { entrypoint_id: "CEP-098", route_file: "src/app/cases/page.tsx", stable_entry: "/cases", half: "product", reason: "UX Run 1 / D-1.5: the identity's case list, rendered only with more than one case; without a session it sends the visitor to /login.", probes: [{ method: "GET", path: "/cases", expected: "307" }] },
  { entrypoint_id: "CEP-099", route_file: "src/app/api/cases/access/request/route.ts", stable_entry: "/api/cases/access/request", half: "product", reason: "UX Run 1 / U2: issues an access code to the channel on file; an empty body is a 400, never a hint about a contact.", probes: [{ method: "POST", path: "/api/cases/access/request", expected: "400" }] },
  { entrypoint_id: "CEP-100", route_file: "src/app/api/cases/access/verify/route.ts", stable_entry: "/api/cases/access/verify", half: "product", reason: "UX Run 1 / U2: turns a valid code into a rolling identity session; an empty body is a 400.", probes: [{ method: "POST", path: "/api/cases/access/verify", expected: "400" }] },
  { entrypoint_id: "CEP-101", route_file: "src/app/api/cases/access/resend/route.ts", stable_entry: "/api/cases/access/resend", half: "product", reason: "UX Run 1 / U5: re-sends the case link for the funnel cookie's case; without the cookie it answers 401.", probes: [{ method: "POST", path: "/api/cases/access/resend", expected: "401" }] },
