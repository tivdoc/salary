...
export const ROUTE_SPLIT: readonly RouteAssignment[] = Object.freeze([
  ...
  { entrypoint_id: "CEP-022", route_file: "src/app/api/payments/start/route.ts", rewritten_on_branch: "Site S4 (2.6): the terms consent is recorded here, before the checkout opens, so a payment cannot exist without the agreement that preceded it. The version comes from the server's own constant and never from the body, and the route refuses when the box was not ticked.", stable_entry: "/api/payments/start", half: "product", reason: `${MAIN}: starts an Invoice4u checkout.`, probes: [{ method: "POST", path: "/api/payments/start", expected: "401" }] },
