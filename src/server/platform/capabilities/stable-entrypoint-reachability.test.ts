...
describe("stable product dispatcher capability reachability", () => {
  ...
  it("maps the exact 47 live Next roots plus the canonical route registrar", () => {
    ...
    expect(nextRoots).toHaveLength(47);
    expect(registrar).toHaveLength(1);
    expect(new Set(STABLE_PRODUCT_DISPATCHER_ROOTS.map((entry) => entry.entrypoint_id)).size).toBe(48);
  ...
  it("proves every app dispatcher invokes its exact request-time capability guard", () => {
    ...
    expect(appRoots).toHaveLength(23);
  ...
  it("proves every exported HTTP method reaches its exact bounded request guard", () => {
    ...
    expect(apiRoots).toHaveLength(24);
  ...
  it("proves the registrar root installs and immediately verifies its exact guard", () => {
