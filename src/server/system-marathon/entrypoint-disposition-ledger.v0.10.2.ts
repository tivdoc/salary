...
export type EntrypointDispositionLedger = Readonly<{
  ...
  denominator: 113;
  product_stable_denominator: 102;
...
export function validateEntrypointDispositionLedger(
  ...
  if (ledger.denominator !== 113 || ledger.rows.length !== 113) issues.push("DISPOSITION_DENOMINATOR_CHANGED");
  if (ledger.product_stable_denominator !== 102 || ledger.rows.filter((row) => row.product_stable).length !== 102) {
