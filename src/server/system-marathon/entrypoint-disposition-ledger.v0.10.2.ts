...
export type EntrypointDispositionLedger = Readonly<{
  ...
  denominator: 111;
  product_stable_denominator: 100;
...
export function validateEntrypointDispositionLedger(
  ...
  if (ledger.denominator !== 111 || ledger.rows.length !== 111) issues.push("DISPOSITION_DENOMINATOR_CHANGED");
  if (ledger.product_stable_denominator !== 100 || ledger.rows.filter((row) => row.product_stable).length !== 100) {
