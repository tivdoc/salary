...
export function validateEntrypointDispositionLedger(
  ...
  if (ledger.denominator !== 110 || ledger.rows.length !== 110) issues.push("DISPOSITION_DENOMINATOR_CHANGED");
  if (ledger.product_stable_denominator !== 99 || ledger.rows.filter((row) => row.product_stable).length !== 99) {
