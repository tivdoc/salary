...
export function validateEntrypointDispositionLedger(
  ...
  if (ledger.denominator !== 108 || ledger.rows.length !== 108) issues.push("DISPOSITION_DENOMINATOR_CHANGED");
  if (ledger.product_stable_denominator !== 97 || ledger.rows.filter((row) => row.product_stable).length !== 97) {
