...
describe("the draft shadow run", () => {
  it("computes every case of the corpus through every spec it names, deterministically", () => {
    ...
    expect(first.counts).toMatchObject({ cases: SYNTHETIC_CORPUS.length, executions: 87, ran: 60, preparation_refused: 23, executor_refused: 4, active_real_parameter_count: 0, monetary_output_count: 0, finding_count: 0, customer_report_count: 0 });
  ...
  it("runs every branch of every open decision when asked", () => {
    ...
    expect(all.counts.executions).toBeGreaterThan(87);
  });
});
