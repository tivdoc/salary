...
export async function extractSavedPayslip(input:{caseId:string;versionId:string;expectedMonth:string;context:SnapshotResolutionContext;db:UploadExtractionDb;storage:UploadExtractionStorage;extractor?:OpenAiPayslipV2PassExtractor}){
 ...
 const run=await runOpenAiPayslipExtractionV21({request:{declared_document_type:'payslip',extraction_id:input.context.snapshot_id,analysis_run_id:input.context.analysis_run_id,case_id:input.caseId,document:loaded.document,requested_at:input.context.created_at},source:loaded.source,extractor:input.extractor??createOpenAiPayslipV21ExtractorFromEnv(),snapshot_context:input.context});
