...
function fixture(kind:'reported_work_hours'|'balance_unit'='reported_work_hours'){
 ...
 const target=()=>documentSourceTranscriptionTarget({checkpoint,policyVersion:'source-transcription-test',subject:kind==='reported_work_hours'?{kind,page:1}:{kind,candidateId:balance.candidate_id}});
 const input=()=>({target:target(),currentCheckpoint:checkpoint,caseId:document.case_id,month:'2025-01',policyVersion:'source-transcription-test',requestId:randomUUID(),answerRevision:1,identityId:randomUUID(),answeredAt:'2025-02-02T00:00:00Z',
  answer:{schema_version:'document-field-answer-v2',action:'correct',corrected_raw_value:kind==='reported_work_hours'?'97.5':'days'}});
 const annotated=()=>{
