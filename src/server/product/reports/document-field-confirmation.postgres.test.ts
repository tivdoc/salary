...
it.skipIf(process.env.TIVDOC_FIELD_CONFIRMATION_DB_PROOF!=='1')('binds saved customer readings to actual worker/identity, immutable extraction and purchased month',async()=>{
 ...
 const topicProof=process.env.TIVDOC_FIELD_TOPIC_DB_PROOF==='1';
 if(topicProof)extraction.fields.find(f=>f.field==='pension_base')!.confidence=0.6;
 ...
 const tenant=`saved-case:${caseId}`,sid=`field-proof:${randomUUID()}`,jti=randomUUID(),checks:string[]=[],topicChecks:string[]=[];
 ...
 try{
  ...
  // The fixture inserts a paid row directly. Real checkout captures this input
  // on the payment state transition; capture that same purchased scope here.
  await owner.query("select private.capture_case_input($1,'synthetic_paid_field_scope')",[caseId]);
  ...
  if(topicProof){
   const pension=extraction.fields.find(f=>f.field==='pension_base')!;
   const pensionTarget=documentFieldTarget({checkpoint,policyVersion:SAVED_EXTRACTION_POLICY,candidateId:pension.candidate_id});
   const {proveDocumentFieldTopicScope}=await import('./document-field-topic.postgres-proof');
   await proveDocumentFieldTopicScope({owner,worker,web,caseId,orderId,checkpoint,salaryTarget:target,pensionTarget,head,saved,transact,open,checks:topicChecks});
  }
