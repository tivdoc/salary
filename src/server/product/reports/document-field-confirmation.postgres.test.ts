...
it.skipIf(process.env.TIVDOC_FIELD_CONFIRMATION_DB_PROOF!=='1')('binds saved customer readings to actual worker/identity, immutable extraction and purchased month',async()=>{
 ...
 const checkpoint={schema_version:'tivdoc-saved-extraction-v1',case_id:caseId,product_document_id:docId,version_id:doc.document_id,input_sha256:doc.content_sha256,expected_month:'2025-02',period_mismatch:false,requires_confirmation:true,
 ...
 writeFileSync(`../release-work/field-confirmation-owned-${caseId}.json`,JSON.stringify({caseIds:[caseId,otherId],identities,tenant,sid,docId,orderId,scope:'Owned isolated DEV metadata only; no Storage object created'}));
 ...
 const saved=async()=>{const h=await head();await owner.query(`insert into private.case_extraction_checkpoints(case_id,revision,version_id,input_sha256,policy_version,result_sha256,result)
  values($1,$2,$3,$4,$5,$6,$7) on conflict do nothing`,[caseId,h.revision,doc.document_id,doc.content_sha256,SAVED_EXTRACTION_POLICY,checkpoint.result_sha256,checkpoint]);
 ...
 try{
  ...
  await owner.query("insert into public.documents(id,case_id,version_id,document_type,slot,storage_path,original_filename,mime_type,size,content_sha256,period_month) values($1,$2,$3,'payslip','payslip-01',$4,'synthetic.pdf','application/pdf',100,$5,'2025-02-01')",[docId,caseId,doc.document_id,`cases/${caseId}/versions/${doc.document_id}.pdf`,doc.content_sha256]);
  ...
  const source=(await web.query('select public.case_request_document_source($1,$2,$3) value',[caseId,identities[0],id])).rows[0].value;expect(source.version).toBe(doc.document_id);expect(source.sha256).toBe(doc.content_sha256);
  ...
  await web.query('select public.case_request_edit($1,$2,$3,$4,1,\'correction\')',[caseId,id,identities[0],DOCUMENT_FIELD_CONFIRMATION_ANSWERS[1]]);
