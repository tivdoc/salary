...
it.skipIf(process.env.TIVDOC_QUOTE_CANCEL_DB_PROOF!=='1')('cancels only unstarted quoted orders and preserves credit across retries and checkout races',async()=>{
 ...
 try{
  ...
  const checkout=(orderId:string,terms:string)=>web.query('select public.case_order_checkout_begin($1,$2,$3,$4,$5,$6) value',[ids[0],identities[0],orderId,randomUUID(),canonicalSha256(randomUUID()),terms]);
  ...
  checks.push('foreign identity/case and actual non-worker roles cannot cancel or release a credit');
  ...
  const cancelled=await Promise.all([cancel(worker,original.id),cancel(peer,original.id)]);expect(cancelled.map(r=>r.replayed).sort()).toEqual([false,true]);
  ...
  const race=await Promise.allSettled([cancel(peer,raceOrder.id),checkout(raceOrder.id,raceQuote.termsVersion)]);
  ...
  checks.push('actual web checkout racing worker cancellation cannot both create a provider attempt and release credit');
