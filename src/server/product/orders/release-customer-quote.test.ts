...
import {createReleaseInitialOrder,customerReleaseQuote,customerSavedReleaseQuote} from './service';
...
const sessionToken='s'.repeat(22);
function fixture(){
 ...
 const rpc=vi.fn(async(name:string,_args:Readonly<Record<string,unknown>>):Promise<unknown[]>=>{void _args;if(name==='case_order_release_quote')return [{value:row}];if(name==='case_order_saved_release_quote')return [{value:{quote:null,availability:{state:'not_prepared',period:null}}}];if(name==='case_order_get')return [{value:order}];throw Error('UNEXPECTED_RPC');});
 ...
 const input={caseId,identityId,sessionToken,request:{kind:'full' as const,from:'2026-06',to:'2026-06'}};
...
it('refuses absent pricing evidence without creating a quote or order',async()=>{
 const f=fixture();f.rpc.mockResolvedValueOnce([{value:null}]);await expect(customerReleaseQuote(f.input,f.db)).rejects.toThrow('ORDER_PRICING_BASIS_UNAVAILABLE');expect(f.rpc).toHaveBeenCalledTimes(2);
});
...
it('discovers the actual saved offer period through an authenticated read without caller period selection',async()=>{
 const f=fixture();f.rpc.mockResolvedValueOnce([{value:{quote:f.row,availability:{state:'ready',period:{from:'2026-06',to:'2026-06'}}}}]);
 const result=await customerSavedReleaseQuote({caseId,identityId,sessionToken},f.db);expect(result.quote?.from).toBe('2026-06');
 expect(f.rpc).toHaveBeenCalledExactlyOnceWith('case_order_saved_release_quote',{target_case:caseId,target_identity:identityId,target_session_hash:hashSession(sessionToken)});
 expect(JSON.stringify(result)).not.toContain('input_sha256');
});
it.each(['needs_information','conditional_result','comparison_needs_review','coverage_unavailable','order_needs_review','quote_expired','below_upgrade_threshold','not_prepared'])('retains current %s outcome without fabricating an offer',async state=>{
 const f=fixture(),availability={state,period:{from:'2026-06',to:'2026-06'}};f.rpc.mockResolvedValueOnce([{value:{quote:null,availability}}]);
 expect(await customerSavedReleaseQuote({caseId,identityId,sessionToken},f.db)).toEqual({quote:null,order:null,availability});expect(f.rpc).toHaveBeenCalledOnce();
});
it.each(['ready_without_quote','different_period','foreign_identity','expired_quote','internal_reason'])('rejects %s discovery metadata',async defect=>{
 const f=fixture(),value:{quote:unknown;availability:Record<string,unknown>}={quote:f.row,availability:{state:'ready',period:{from:'2026-06',to:'2026-06'}}};
 if(defect==='ready_without_quote')value.quote=null;if(defect==='different_period')value.availability.period={from:'2026-07',to:'2026-07'};
 if(defect==='foreign_identity')f.row.snapshot={...f.row.snapshot,identity_id:orderId};if(defect==='expired_quote')f.row.now=f.quote.expires_at;
 if(defect==='internal_reason')value.availability.reason='pricing_adapter_unsupported_topics';f.rpc.mockResolvedValueOnce([{value}]);
 await expect(customerSavedReleaseQuote({caseId,identityId,sessionToken},f.db)).rejects.toThrow();
});

it('distinguishes an unmatched requested period only when another actual saved offer exists',async()=>{
 const f=fixture();f.input.request.from='2026-07';f.input.request.to='2026-07';
 f.rpc.mockResolvedValueOnce([{value:null}]).mockResolvedValueOnce([{value:{quote:f.row,availability:{state:'ready',period:{from:'2026-06',to:'2026-06'}}}}]);
 await expect(customerReleaseQuote(f.input,f.db)).rejects.toThrow('ORDER_QUOTE_PERIOD_UNAVAILABLE');expect(f.rpc).toHaveBeenCalledTimes(2);
});

it('refuses discovery without an opaque authenticated session before querying',async()=>{const f=fixture();await expect(customerSavedReleaseQuote({caseId,identityId,sessionToken:''},f.db)).rejects.toThrow('ORDER_FORBIDDEN');expect(f.rpc).not.toHaveBeenCalled();});
