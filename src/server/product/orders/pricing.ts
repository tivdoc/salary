...
/** Explicit release-only basis; historical unversioned payloads retain their vocabulary and hash. */
export const RELEASE_PRICING_BASIS_VERSION='tivdoc-release-pricing-basis-v1' as const;
export const releasePricingBasisSchema=pricingBasisSchema.extend({
 schema_version:z.literal(RELEASE_PRICING_BASIS_VERSION),checked_topics:releasePurchaseTopicsSchema,
 components:z.array(pricingBasisSchema.shape.components.element.extend({topic:z.enum(RELEASE_PURCHASE_TOPICS)}).strict()),
}).strict();
export const savedReleasePricingBasisSchema=z.union([pricingBasisSchema,releasePricingBasisSchema]);
export type SavedReleasePricingBasis=z.infer<typeof savedReleasePricingBasisSchema>;
export function correctionPricingBasisSchema(quote:PriceQuote){
 return quote.schema_version==='tivdoc-price-quote-v2'&&quote.basis_schema_version===RELEASE_PRICING_BASIS_VERSION
  ?releasePricingBasisSchema:pricingBasisSchema;
}
...
export function priceSavedBasis(raw:unknown):PricingResult{
 const parsed=pricingBasisSchema.safeParse(raw);return parsed.success?priceUnderPolicy(parsed.data,productOffer().full_report.pricing):{state:'amount_unknown',reason:'invalid_saved_basis'};
}
export function priceSavedReleaseBasis(raw:unknown):PricingResult{
 const parsed=savedReleasePricingBasisSchema.safeParse(raw);return parsed.success?priceUnderPolicy(parsed.data,productOffer().full_report.pricing):{state:'amount_unknown',reason:'invalid_saved_basis'};
}
function priceUnderPolicy(basis:SavedReleasePricingBasis,policy:PriceQuote['pricing_policy']):PricingResult{
 ...
 for(const c of basis.components){
  ...
  if(!basis.checked_months.includes(c.month)||!(basis.checked_topics as readonly string[]).includes(c.topic))return {state:'amount_unknown',reason:'component_outside_checked_scope'};
...
type QuoteInput={basis:SavedReleasePricingBasis;caseId:string;identityId:string;from:string;to:string;topics:readonly string[];credit:InitialCredit;now:Date};
export function createPriceQuote(input:Omit<QuoteInput,'basis'>&{basis:PricingBasis}){
 return createQuote({...input,basis:pricingBasisSchema.parse(input.basis)},false);
}
export function createReleasePriceQuote(input:QuoteInput){
 return createQuote({...input,basis:savedReleasePricingBasisSchema.parse(input.basis),topics:releasePurchaseTopicsSchema.parse(input.topics)},true);
}
function createQuote(input:QuoteInput,release:boolean){
 ...
 const result=priceUnderPolicy(basis,productOffer().full_report.pricing);if(result.state!=='eligible')return result;
 ...
 if(!release)return priceQuoteSchema.parse({...snapshot,sha256:canonicalSha256(snapshot)});
 const released={...snapshot,schema_version:'tivdoc-price-quote-v2' as const,purchase_topics_version:PURCHASE_TOPICS_VERSION,
  ...('schema_version'in basis?{basis_schema_version:basis.schema_version}:{})};
 return priceQuoteSchema.parse({...released,sha256:canonicalSha256(released)});
}
...
export function quoteCorrectionRefund(candidateQuote:unknown,candidateBasis:unknown){
 const parsedQuote=priceQuoteSchema.safeParse(candidateQuote);
 const parsedBasis=parsedQuote.success?correctionPricingBasisSchema(parsedQuote.data).safeParse(candidateBasis):null;
