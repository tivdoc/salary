...
export type ProductOrder={id:string;case_id:string;kind:'initial'|'full';period_from:string;period_to:string;amount_minor:number;currency:'ILS';state:'awaiting_payment'|'paid'|'cancelled';refund_state:'none'|'requested'|'processing'|'refunded'|'rejected';offer:ReturnType<typeof offerSnapshot>|QuotedFullOffer;terms_version:string;topics:string[];created_at:string;verified_at:string|null;published_at:string|null;checkout_state:'none'|'creating'|'ready'|'uncertain';checkout_url:string|null;payment_id:string|null;receipt_url:string|null;price_correction?:z.infer<typeof priceCorrectionStatusSchema>|null;};
