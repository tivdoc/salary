...
async function fixture(paidHourly=false,missingHours=false){
 const original=await devFinancialInputFixture(missingHours);
 const output=paidHourly?{...original.output,payroll_rows:[{...original.output.payroll_rows[1],amount_raw:'3300.00'}]}:original.output;
...
describe('engineering financial source permits exactly one paid base observation',()=>{
 ...
 it.each(['second_paid','bonus','wrong_amount','wrong_version','uncertain','warning','incomplete','percentage'] as const)('refuses %s',async change=>{
  let source=await fixture(true);const row=source.additional_components[0];
  ...
  if(change==='incomplete')source={...source,earnings_components_complete:false};
