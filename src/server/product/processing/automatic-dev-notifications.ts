...
export async function runAutomaticNotificationPass(input:{db:CaseAccessDb;capability:string;secret:string;origin:string;provider:NotificationProvider;enqueueEventKeys?:readonly string[];signal?:AbortSignal}){
 ...
 for(const event of events.filter(event=>event.event_kind!=='request_required'&&(input.enqueueEventKeys===undefined||input.enqueueEventKeys.includes(event.event_key)))){
  ...
  const engineering=event.event_kind==='engineering_report_ready';
  const link=`${url.origin}/case/${event.public_id}/reports?${engineering?'engineering=1&':''}report=${event.report_id}`;
  const message:NotificationMessage={template:'report_ready',channel:'email',to:event.contact,
   subject:engineering?`Tivdoc DEV — דוח ניסוי הנדסי בתיק ${event.public_id}`:`Tivdoc DEV — דוח בדיקה סינתטי בתיק ${event.public_id} זמין`,
   body:[engineering?'נוצר דוח ניסוי הנדסי ממסמך סינתטי. הכלל אינו פעיל בשירות וזה אינו חוב מאומת של מעסיק.':'נוצר דוח בדיקה בסביבת DEV מתיק וממסמך סינתטיים. הדוח נועד לאימות המסלול בלבד; הוא אינו אישור אנושי, אינו חוב מאומת של מעסיק ואינו מעיד שהכלל פעיל בשירות ללקוחות.',`לצפייה מאובטחת: ${link}`,'הקישור דורש כניסה לתיק דרך האימייל המאומת.'].join('\n')};
