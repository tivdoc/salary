...
import type {readManagedDevStatus,retryManagedDevJob} from './managed-worker-host';
...
export function createManagedWorkerHttpHandler(input:{enabled:boolean;sessions:Pick<ProductSessionBoundary,'verify'>;
 read:typeof readManagedDevStatus;retry:typeof retryManagedDevJob;ownerId?:string}){
 return async(request:Request)=>{
  ...
  if(request.method==='GET'){
   try{return productJson({data:await input.read()});}catch{return productJson({code:'MANAGED_DEV_STATUS_UNAVAILABLE'},503);}
  }
