...
export function overviewFrom(requests:PromiseSettledResult<readonly StoredRequest[]>,reports:PromiseSettledResult<CustomerReports>,now:number):CaseOverview{
 const open=requests.status==='fulfilled'?requests.value.filter(r=>r.answered_at===null&&r.source_current!==false&&!r.not_required_for_current_review&&!r.covered_by_field_request_id&&!(r.document_upload_state?.state==='satisfied'&&r.document_upload_state.information_satisfied)&&Date.parse(r.expires_at)>now):null;
...
export async function loadCaseOverview(item:{case_id:string;public_id:string},identityId:string){const [requests,reports]=await Promise.allSettled([listCaseRequests(item.case_id,undefined,identityId),customerReports(item.case_id,identityId,item.public_id)]);return overviewFrom(requests,reports,Date.now());}
