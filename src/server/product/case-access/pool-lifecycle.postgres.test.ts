...
it.skipIf(process.env.TIVDOC_POOL_DB_PROOF!=='1').each(['5432','6543'])('cold concurrent RPCs share a real TLS pool on %s, drain idle TCP clients, then reconnect',async(port)=>{
 ...
 expect(url.hostname).toBe('aws-0-eu-central-1.pooler.supabase.com');expect(url.port).toBe('5432');expect(url.pathname).toBe('/tivdoc_release_replay_20260907');expect(decodeURIComponent(url.username)).toBe('tivdoc_web_runtime.cpzrbidxftzqcfeqqusu');url.port=port;url.search='?sslmode=verify-full';
