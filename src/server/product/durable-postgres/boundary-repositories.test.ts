...
import { DURABLE_BOUNDARY_SQL_TEXT, durableBoundaryStatements } from "./boundary-sql.ts";
...
const OBJECT_ID = `object_${"c".repeat(48)}`;
...
describe("durable PostgreSQL product boundaries", () => {
  ...
  it("returns only exact approved bytes and rejects length/hash corruption", async () => {
    ...
    const repository = new PostgresPrivateReportObjectRepository(callbackClient(() => result([approvedObjectRow({
      byte_length: bytes.byteLength,
      artifact_sha256: artifactSha,
    })])));
    const exact = new DurableApprovedReportObjectReader(repository, blobProvider(async () => Uint8Array.from(bytes)));
    await expect(exact.download(reportReadInput({ artifact_sha256: artifactSha }))).resolves.toMatchObject({ bytes });

    const corrupt = new DurableApprovedReportObjectReader(repository, blobProvider(async () => Uint8Array.from([1, 2, 3, 9])));
    await expect(corrupt.download(reportReadInput({ artifact_sha256: artifactSha }))).rejects.toMatchObject({
    ...
    await expect(short.download(reportReadInput({ artifact_sha256: artifactSha }))).rejects.toMatchObject({
      code: "DURABLE_BOUNDARY_REPORT_INTEGRITY_FAILURE",
    });
  });
  ...
  it("binds immutable report locator/hash/length metadata and requires explicit approval/revocation", async () => {
    ...
    const repository = new PostgresPrivateReportObjectRepository(callbackClient((query) => {
      statements.push(query.name);
      if (query.name === "product_report_bind") return result([reportObjectRow()]);
      return result([{ accepted: true }]);
    }));
    await expect(repository.bind(reportBindInput())).resolves.toMatchObject({
      state: "staged",
      grant_epoch: 0,
      provider_locator: LOCATOR,
      artifact_sha256: HASH_B,
    });
    await repository.approve({ tenant_id: TENANT, case_id: CASE, object_version_id: OBJECT_ID, expected_grant_epoch: 0 });
...
function reportBindInput(): Parameters<PostgresPrivateReportObjectRepository["bind"]>[0] {
  return {
    tenant_id: TENANT,
    case_id: CASE,
