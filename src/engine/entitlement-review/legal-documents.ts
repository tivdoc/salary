...
export function entitlementLegalDocuments(caseId:string,topics:readonly string[]=['working_time','pension','minimum_wage','travel','vacation','convalescence'],options?:{obligations_policy?:typeof OBLIGATIONS_CASE_POLICY;travel_policy?:typeof TRAVEL_GENERAL_ORDER_FLOOR_POLICY}):ReviewDocument[]{
...
export function isPinnedEntitlementLegalDocument(candidate:unknown,caseId:string):boolean{
 return isPinnedNightEntitlementLegalDocument(candidate,caseId)||[...entitlementLegalDocuments(caseId),...obligationLegalDocuments(caseId),...entitlementLegalDocuments(caseId,['travel'],{travel_policy:TRAVEL_GENERAL_ORDER_FLOOR_POLICY})].some(d=>canonicalSha256(d)===canonicalSha256(candidate));
}
