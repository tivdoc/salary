...
export function entitlementLegalDocuments(caseId:string,topics:readonly string[]=['working_time','pension','minimum_wage','travel','vacation','convalescence'],options?:{obligations_policy?:typeof OBLIGATIONS_CASE_POLICY}):ReviewDocument[]{
...
export function isPinnedEntitlementLegalDocument(candidate:unknown,caseId:string):boolean{
 return isPinnedNightEntitlementLegalDocument(candidate,caseId)||[...entitlementLegalDocuments(caseId),...obligationLegalDocuments(caseId)].some(d=>canonicalSha256(d)===canonicalSha256(candidate));
}
