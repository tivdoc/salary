...
function resolve(input:DocumentReviewInput,e:EntitlementEvidence):EntitlementBranchReview{
 const parts=[...(e.pension===undefined?[]:[pensionProductReview(input,e.pension)]),...(e.working_time===undefined?[]:[workingTimeProductReview(input,e.working_time)])];
 return {checks:parts.flatMap(p=>p.checks),gaps:parts.flatMap(p=>p.gaps),needs:parts.flatMap(p=>p.needs),answer_targets:parts.flatMap(p=>p.answer_targets),selections:parts.flatMap(p=>p.selections)};
}
function factAt(e:EntitlementEvidence,target:EntitlementAnswerTarget):Record<string,unknown>{
 let current:unknown=target.branch==='pension'?e.pension:(e.working_time as unknown[])[target.index!];
...
function answeredEvidence(input:DocumentReviewInput,original:EntitlementEvidence,targets:EntitlementAnswerTarget[]):EntitlementEvidence{
 ...
 for(const target of targets){
  ...
  const fact=factAt(packet,target),known=r.state==='provided'&&!admitted.blocked;
  fact.state=known?(target.branch==='pension'?'known':'declared'):r.state==='conflicted'||admitted.state==='provided'&&admitted.blocked?'conflict':'unknown';
  fact.value=known?(target.value_kind==='date_or_ongoing'&&r.value==='העבודה נמשכת'?'ongoing':r.value):null;
  ...
  if(target.branch==='pension')fact.basis='customer_declaration';
  const branch=(target.branch==='pension'?packet.pension:(packet.working_time as unknown[])[target.index!]) as {source_manifest:{document_id:string;version_id:string;file_sha256:string;page_count:number;kind:string;case_id:string|null}[]};
...
export function composeEntitlementReview(candidate:DocumentReviewInput):DocumentReviewInput{
 ...
 const laws=entitlementLegalDocuments(base.case_id),packet=assertEntitlementSourcePacket(base,base.entitlement_evidence,laws);
 ...
 const checks=current.checks.map(check=>{
  ...
  return {...check,calculation:{...calc,source_manifest}};
 });
 ...
 for(const need of needs)for(const id of need.dependent_check_ids)if(!checks.some(c=>c.check_id===id)&&!gaps.some(g=>g.check_id===id)){
 ...
 const body={policy_version:ENTITLEMENT_REVIEW_POLICY,source_evidence_sha256:canonicalSha256(packet),evidence:effective,selections,generated_fact_keys:needs.map(n=>n.fact_key)};
