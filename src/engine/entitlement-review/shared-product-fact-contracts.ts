...
export const SHARED_PERSONAL_FACTS_EXPANDED_POLICY='shared-personal-facts-v3' as const;
export const sharedPersonalFactsPolicySchema=z.enum([SHARED_PERSONAL_FACTS_POLICY,SHARED_PERSONAL_FACTS_TRAVEL_POLICY,SHARED_PERSONAL_FACTS_EXPANDED_POLICY]);
export const sharedPersonalFactSchema=z.enum(['birth_date','employment_relationship','workplace_sector']);
const legacySharedPersonalFactGroupSchema=z.object({schema_version:z.literal('shared-personal-fact-group-v1'),policy_version:z.enum([SHARED_PERSONAL_FACTS_POLICY,SHARED_PERSONAL_FACTS_TRAVEL_POLICY]),
...
const expandedSharedPersonalFactGroupSchema=z.object({schema_version:z.literal('shared-personal-fact-group-v2'),policy_version:z.literal(SHARED_PERSONAL_FACTS_EXPANDED_POLICY),
 case_id:z.string().min(1),period:z.object({from:z.iso.date(),to:z.iso.date()}).strict(),fact:sharedPersonalFactSchema,
 canonical_fact_key:z.string().min(1),canonical_target_sha256:hash.nullable(),
 state:z.enum(['missing','provided','unknown','unreadable','conflict']),
 origin:z.discriminatedUnion('kind',[
  z.object({kind:z.literal('none')}).strict(),
  z.object({kind:z.literal('answer'),request_id:z.uuid(),target_sha256:hash,answer_sha256:hash,source_sha256:hash}).strict(),
  z.object({kind:z.literal('source_fact'),branch:z.enum(['minimum_wage','pension','travel','convalescence','vacation','working_time']),index:z.number().int().min(0).max(5).nullable(),input_path:z.string(),fact_sha256:hash,source_sha256:hash}).strict(),
 ]),
 value_sha256:hash.nullable(),source_sha256s:z.array(hash).max(24),current_source_pins_sha256:hash,
 aliases:z.array(z.object({branch:z.enum(['minimum_wage','pension','travel','convalescence','vacation','working_time']),index:z.number().int().min(0).max(5).nullable(),input_path:z.string(),original_fact_sha256:hash,
  fact_key:z.string(),target_sha256:hash.nullable(),dependent_check_ids:z.array(z.string()).max(128)}).strict()).min(1).max(11),
 historical_target_sha256s:z.array(hash).max(24),group_sha256:hash,
}).strict().refine(v=>{const {group_sha256,...body}=v;return group_sha256===canonicalSha256(body);},'SHARED_PERSONAL_GROUP_HASH').refine(v=>v.aliases.every(a=>(a.branch==='working_time')===(a.index!==null))&&(v.origin.kind!=='source_fact'||(v.origin.branch==='working_time')===(v.origin.index!==null)),'SHARED_PERSONAL_INDEX_SCOPE');
export const sharedPersonalFactGroupSchema=z.union([legacySharedPersonalFactGroupSchema,expandedSharedPersonalFactGroupSchema]);
