...
export const PENSION_TABLE_SOURCE_POLICY='pension-identified-table-v1' as const;
...
export const pensionTablePercentSchema=z.string().max(80).regex(/^\d+(?:\.\d+)?$/u).refine(v=>Number(v)<=100,'PENSION_TABLE_PERCENT');
...
export const pensionSourceFactsSchema=z.object({schema_version:z.literal(PENSION_SOURCE_FACTS_POLICY),
 ...
 table_policy:z.literal(PENSION_TABLE_SOURCE_POLICY).optional(),
 /** Observed table rates do not identify the selected pension vehicle or the
  * complete current pensionable wage. A temporal assertion remains separate. */
 arrangement_table:fact(z.object({employee_percent:pensionTablePercentSchema,employer_percent:pensionTablePercentSchema,severance_percent:pensionTablePercentSchema,
  product:z.literal('unspecified_pension_product'),source_group_sha256:hash,period:period.nullable(),
  temporal_association:z.enum(['unresolved','identified_source_dates','declared_no_change','change_reported','conflict','unknown','unreadable','stale','expired']),
  temporal_fact_sha256:hash.nullable()}).strict()).optional(),
}).strict().refine(v=>(v.table_policy===undefined)===(v.arrangement_table===undefined),'PENSION_TABLE_POLICY_REQUIRED');
export type PensionSourceFacts=z.infer<typeof pensionSourceFactsSchema>;
export function emptyPensionSourceFacts(options:{tables?:boolean}={}):PensionSourceFacts{const missing={state:'missing',value:null,source:null};return pensionSourceFactsSchema.parse({schema_version:PENSION_SOURCE_FACTS_POLICY,
 wage_basis:missing,eligible_interval_basis:missing,prior_insurance:missing,arrangement:missing,...(options.tables?{table_policy:PENSION_TABLE_SOURCE_POLICY,arrangement_table:missing}:{})});}
