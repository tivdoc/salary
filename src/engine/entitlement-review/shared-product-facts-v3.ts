...
type Branch='minimum_wage'|'pension'|'travel'|'convalescence'|'vacation'|'working_time';
...
type Alias={branch:Branch;index:number|null;key:Key;path:string;fact:Fact;fact_key:string;need:ReviewCompletionNeed|undefined;request:ReviewCompletion|undefined;ids:string[]};
...
const order:Branch[]=['minimum_wage','pension','travel','convalescence','vacation','working_time'];
const enabled=(e:EntitlementEvidence)=>e.shared_personal_facts_policy===SHARED_PERSONAL_FACTS_EXPANDED_POLICY;
...
function branchInput(e:EntitlementEvidence,branch:Branch,index:number|null){
 if(branch==='working_time'){if(index===null)throw Error('SHARED_PERSONAL_WEEK_INDEX');return workingTimeEntitlementInputSchema.array().min(1).max(6).parse(e.working_time)[index];}
 return branch==='pension'?pensionEntitlementInputSchema.parse(e.pension):branch==='travel'?travelEntitlementInputSchema.parse(e.travel):branch==='vacation'?vacationEntitlementInputSchema.parse(e.vacation):branch==='convalescence'?convalescenceEntitlementInputSchema.parse(e.convalescence):minimumWageEntitlementInputSchema.parse(e.minimum_wage);
}
function branchReview(input:DocumentReviewInput,e:EntitlementEvidence,branch:Branch){return branch==='working_time'?workingTimeProductReview(input,e.working_time):branch==='pension'?pensionProductReview(input,e.pension):simpleEntitlementProduct(input,branch,e[branch]);}
...
function inventory(input:DocumentReviewInput,e:EntitlementEvidence):Alias[]{const aliases:Alias[]=[];
 for(const branch of order){if(e[branch]===undefined)continue;const part=branchReview(input,e,branch);const indices:(number|null)[]=branch==='working_time'?workingTimeEntitlementInputSchema.array().min(1).max(6).parse(e.working_time).map((_,i)=>i):[null];for(const index of indices){const b=branchInput(e,branch,index);if(!b?.product_facts)continue;
  const pins=input.documents.filter(d=>b.source_manifest.some(m=>m.kind==='case_document'&&m.document_id===d.document_id)).map(d=>({case_id:d.case_id,document_id:d.document_id,version_id:d.version_id,source_sha256:d.file_sha256}));
  for(const key of keys){if(!(key in b.product_facts))continue;const path='product_facts.'+key;
   const fact_key=branch==='working_time'?workingTimeProductFactKey(input,workingTimeEntitlementInputSchema.parse(b),path):branch==='pension'?`entitlement.pension.${canonicalSha256({period:input.period,pins,path}).slice(0,32)}`
   ...
   aliases.push({branch,index,key,path,fact,fact_key,need,request:need?requestFor(input,need):undefined,
...
function observedSourceCurrent(input:DocumentReviewInput,f:Fact){
 if(f.state==='declared'&&f.source?.reading==='questionnaire_declaration'&&f.value!==null){assertQuestionnaireSource(input,f.source,f.value);return true;}
 return f.state==='observed'&&f.value!==null&&['identified_document_reading','provider_extraction'].includes(f.source?.reading??'')
...
function groups(input:DocumentReviewInput,e:EntitlementEvidence):ResolvedGroup[]{
 ...
 for(const key of keys){let aliases=all.filter(a=>a.key===key),needed=aliases.filter(a=>a.request);
  ...
  // acquire an identified-reading label by crossing a shared alias.
  if(!chosen&&!rawBlocked&&!conflict&&observations[0]?.fact.source?.reading==='provider_extraction'){
   aliases=aliases.filter(a=>['minimum_wage','convalescence','vacation'].includes(a.branch));needed=aliases.filter(a=>a.request);
  ...
  else if(observation)fact={state:state==='provided'?observation.fact.state:state,value:state==='provided'?observation.fact.value:null,source:observation.fact.source};
  if(state==='provided'&&!fact){state='missing';fact=null;}
  const origin:ResolvedGroup['receipt']['origin']=rawBlocked?(rawBlocked.fact.source?{kind:'source_fact',branch:rawBlocked.branch,index:rawBlocked.index,input_path:rawBlocked.path,fact_sha256:canonicalSha256(rawBlocked.fact),source_sha256:hashSource(rawBlocked.fact.source)}:{kind:'none'}):chosen?{kind:'answer',request_id:chosen.h.receipt.request_id,target_sha256:chosen.h.request.target.target_sha256,answer_sha256:chosen.h.receipt.answer_sha256,source_sha256:hashSource(chosen.source)}
   :observation?{kind:'source_fact',branch:observation.branch,index:observation.index,input_path:observation.path,fact_sha256:canonicalSha256(observation.fact),source_sha256:hashSource(observation.fact.source!)}:{kind:'none'};
  const body={schema_version:'shared-personal-fact-group-v2' as const,policy_version:SHARED_PERSONAL_FACTS_EXPANDED_POLICY,case_id:input.case_id,period:input.period,fact:key,
   ...
   aliases:aliases.map(a=>({branch:a.branch,index:a.index,input_path:a.path,original_fact_sha256:canonicalSha256(a.fact),fact_key:a.fact_key,target_sha256:a.request?.target.target_sha256??null,dependent_check_ids:a.ids})),
...
export function materializeSharedPersonalFacts(input:DocumentReviewInput,original:EntitlementEvidence,effective:EntitlementEvidence):EntitlementEvidence{
 ...
 for(const group of groups(input,original))if(group.fact)for(const alias of group.aliases){
  const b=branchInput(result,alias.branch,alias.index);if(!b.product_facts)continue;
  ...
  if(s){const origin=input.documents.find(d=>d.document_id===s.document_id&&d.version_id===s.version_id);
   ...
   b.source_manifest.push({document_id:s.document_id,version_id:s.version_id,file_sha256:s.file_sha256,page_count:origin?.page_count??1,kind:s.reading==='customer_declaration'?'customer_answer':s.reading==='questionnaire_declaration'?'questionnaire':'case_document',case_id:input.case_id});}
  if(alias.branch==='working_time'){const weeks=workingTimeEntitlementInputSchema.array().min(1).max(6).parse(result.working_time);weeks[alias.index!]=workingTimeEntitlementInputSchema.parse(b);result.working_time=weeks;}else result[alias.branch]=b;
 }
 return result;
}
...
export function sharedPersonalAliasFact(input:DocumentReviewInput,packet:EntitlementEvidence,path:string,fact:unknown):boolean{
 ...
 const group=groups(input,original).find(g=>g.aliases.some(a=>`${a.branch}.${a.index===null?'':a.index+'.'}${a.path}`===path));
 if(!group?.fact)return false;
 const alias=group.aliases.find(a=>`${a.branch}.${a.index===null?'':a.index+'.'}${a.path}`===path);if(!alias)return false;
 const expected=materializeSharedPersonalFacts(input,original,original),b=branchInput(expected,alias.branch,alias.index),actual=branchInput(packet,alias.branch,alias.index);
 return !!b?.product_facts&&!!actual?.product_facts&&same(Reflect.get(b.product_facts,alias.key),fact);
}
...
export function assertSharedPersonalMaterialization(input:DocumentReviewInput,packet:EntitlementEvidence){const original=input.entitlement_evidence;
 ...
 for(const group of groups(input,original))if(group.fact)for(const alias of group.aliases){
  const actual=branchInput(packet,alias.branch,alias.index),wanted=branchInput(expected,alias.branch,alias.index);
