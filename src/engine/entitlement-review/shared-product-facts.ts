...
import {SHARED_PERSONAL_FACTS_POLICY,sharedPersonalFactsManifestSchema,type SharedPersonalFactManifest} from './shared-product-fact-contracts.ts';

export {SHARED_PERSONAL_FACTS_POLICY} from './shared-product-fact-contracts.ts';
type Branch='minimum_wage'|'pension';
...
export function enableSharedPersonalFacts(candidate:EntitlementEvidence):EntitlementEvidence{return {...candidate,shared_personal_facts_policy:SHARED_PERSONAL_FACTS_POLICY};}
function branchInput(e:EntitlementEvidence,branch:Branch){return branch==='pension'?pensionEntitlementInputSchema.parse(e.pension):minimumWageEntitlementInputSchema.parse(e.minimum_wage);}
function branchReview(input:DocumentReviewInput,e:EntitlementEvidence,branch:Branch){return branch==='pension'?pensionProductReview(input,e.pension):simpleEntitlementProduct(input,'minimum_wage',e.minimum_wage);}
...
function inventory(input:DocumentReviewInput,e:EntitlementEvidence):Alias[]{const aliases:Alias[]=[];
 for(const branch of order){if(e[branch]===undefined)continue;const b=branchInput(e,branch);if(!b.product_facts)continue;
  ...
  for(const key of keys){if(!(key in b.product_facts))continue;const path='product_facts.'+key;
   const fact_key=branch==='pension'?`entitlement.pension.${canonicalSha256({period:input.period,pins,path}).slice(0,32)}`
    :`entitlement.minimum_wage.${canonicalSha256({period:input.period,pins,path,key:'personal.'+path}).slice(0,28)}`;
...
function groups(input:DocumentReviewInput,e:EntitlementEvidence):ResolvedGroup[]{
 if(e.shared_personal_facts_policy!==SHARED_PERSONAL_FACTS_POLICY)return [];
 ...
 for(const key of keys){const aliases=all.filter(a=>a.key===key),needed=aliases.filter(a=>a.request);
  ...
  const body={schema_version:'shared-personal-fact-group-v1' as const,policy_version:SHARED_PERSONAL_FACTS_POLICY,case_id:input.case_id,period:input.period,fact:key,
...
export function sharedPersonalFactManifest(input:DocumentReviewInput,e:EntitlementEvidence){
 if(e.shared_personal_facts_policy!==SHARED_PERSONAL_FACTS_POLICY)return undefined;
 const body={policy_version:SHARED_PERSONAL_FACTS_POLICY,groups:groups(input,e).map(g=>g.receipt)};
...
export function projectSharedPersonalFactNeeds(input:DocumentReviewInput,e:EntitlementEvidence,part:EntitlementBranchReview):EntitlementBranchReview{
 if(e.shared_personal_facts_policy!==SHARED_PERSONAL_FACTS_POLICY)return part;
...
export function materializeSharedPersonalFacts(input:DocumentReviewInput,original:EntitlementEvidence,effective:EntitlementEvidence):EntitlementEvidence{
 if(original.shared_personal_facts_policy!==SHARED_PERSONAL_FACTS_POLICY)return effective;
...
export function sharedPersonalAliasFact(input:DocumentReviewInput,packet:EntitlementEvidence,path:string,fact:unknown):boolean{
 const original=input.entitlement_evidence;if(original?.shared_personal_facts_policy!==SHARED_PERSONAL_FACTS_POLICY)return false;
 const group=groups(input,original).find(g=>g.aliases.some(a=>`${a.branch}.${a.path}`===path));
 if(!group?.fact)return false;
 const expected=materializeSharedPersonalFacts(input,original,original),match=/^(minimum_wage|pension)\.product_facts\.([a-z_]+)$/u.exec(path);if(!match)return false;
...
export function sharedPersonalAnswerCoversCheck(input:DocumentReviewInput,answerSha:string,checkId:string){
 const e=input.entitlement_evidence;if(e?.shared_personal_facts_policy!==SHARED_PERSONAL_FACTS_POLICY)return false;
...
export function sharedPersonalCanonicalTargets(input:DocumentReviewInput){const e=input.entitlement_evidence;return e?.shared_personal_facts_policy===SHARED_PERSONAL_FACTS_POLICY?groups(input,e).flatMap(g=>g.canonical?[g.canonical]:[]):[];}
export function sharedPersonalAnswerIsCurrent(input:DocumentReviewInput,answerSha:string){const e=input.entitlement_evidence;
 if(e?.shared_personal_facts_policy!==SHARED_PERSONAL_FACTS_POLICY)return false;
...
export function assertSharedPersonalMaterialization(input:DocumentReviewInput,packet:EntitlementEvidence){const original=input.entitlement_evidence;
 if(original?.shared_personal_facts_policy!==SHARED_PERSONAL_FACTS_POLICY||same(original,packet))return;
