import * as expandedSharedFacts from './shared-product-facts-v3.ts';
...
import {SHARED_PERSONAL_FACTS_POLICY,SHARED_PERSONAL_FACTS_TRAVEL_POLICY,SHARED_PERSONAL_FACTS_EXPANDED_POLICY,sharedPersonalFactsManifestSchema,type SharedPersonalFactManifest} from './shared-product-fact-contracts.ts';

export {SHARED_PERSONAL_FACTS_POLICY,SHARED_PERSONAL_FACTS_TRAVEL_POLICY,SHARED_PERSONAL_FACTS_EXPANDED_POLICY} from './shared-product-fact-contracts.ts';
...
function groups(input:DocumentReviewInput,e:EntitlementEvidence):ResolvedGroup[]{
 ...
 for(const key of keys){const aliases=all.filter(a=>a.key===key),needed=aliases.filter(a=>a.request);
  ...
  const body={schema_version:'shared-personal-fact-group-v1' as const,policy_version:e.shared_personal_facts_policy===SHARED_PERSONAL_FACTS_TRAVEL_POLICY?SHARED_PERSONAL_FACTS_TRAVEL_POLICY:SHARED_PERSONAL_FACTS_POLICY,case_id:input.case_id,period:input.period,fact:key,
...
export function sharedPersonalFactManifest(input:DocumentReviewInput,e:EntitlementEvidence){
 if(e.shared_personal_facts_policy===SHARED_PERSONAL_FACTS_EXPANDED_POLICY)return expandedSharedFacts.sharedPersonalFactManifest(input,e);
...
export function projectSharedPersonalFactNeeds(input:DocumentReviewInput,e:EntitlementEvidence,part:EntitlementBranchReview):EntitlementBranchReview{
 if(e.shared_personal_facts_policy===SHARED_PERSONAL_FACTS_EXPANDED_POLICY)return expandedSharedFacts.projectSharedPersonalFactNeeds(input,e,part);
...
export function materializeSharedPersonalFacts(input:DocumentReviewInput,original:EntitlementEvidence,effective:EntitlementEvidence):EntitlementEvidence{
 if(original.shared_personal_facts_policy===SHARED_PERSONAL_FACTS_EXPANDED_POLICY)return expandedSharedFacts.materializeSharedPersonalFacts(input,original,effective);
...
export function sharedPersonalAliasFact(input:DocumentReviewInput,packet:EntitlementEvidence,path:string,fact:unknown):boolean{
 if(input.entitlement_evidence?.shared_personal_facts_policy===SHARED_PERSONAL_FACTS_EXPANDED_POLICY)return expandedSharedFacts.sharedPersonalAliasFact(input,packet,path,fact);
...
export function sharedPersonalAnswerCoversCheck(input:DocumentReviewInput,answerSha:string,checkId:string){
 if(input.entitlement_evidence?.shared_personal_facts_policy===SHARED_PERSONAL_FACTS_EXPANDED_POLICY)return expandedSharedFacts.sharedPersonalAnswerCoversCheck(input,answerSha,checkId);
...
export function sharedPersonalCanonicalTargets(input:DocumentReviewInput){
 if(input.entitlement_evidence?.shared_personal_facts_policy===SHARED_PERSONAL_FACTS_EXPANDED_POLICY)return expandedSharedFacts.sharedPersonalCanonicalTargets(input);const e=input.entitlement_evidence;return e&&enabled(e)?groups(input,e).flatMap(g=>g.canonical?[g.canonical]:[]):[];}
export function sharedPersonalAnswerIsCurrent(input:DocumentReviewInput,answerSha:string){
 if(input.entitlement_evidence?.shared_personal_facts_policy===SHARED_PERSONAL_FACTS_EXPANDED_POLICY)return expandedSharedFacts.sharedPersonalAnswerIsCurrent(input,answerSha);const e=input.entitlement_evidence;
...
export function assertSharedPersonalMaterialization(input:DocumentReviewInput,packet:EntitlementEvidence){
 if(input.entitlement_evidence?.shared_personal_facts_policy===SHARED_PERSONAL_FACTS_EXPANDED_POLICY)return expandedSharedFacts.assertSharedPersonalMaterialization(input,packet);const original=input.entitlement_evidence;
