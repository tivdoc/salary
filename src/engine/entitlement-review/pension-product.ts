...
export function pensionProductReview(input:DocumentReviewInput,candidate:unknown):EntitlementBranchReview{
 ...
 for(const missing of resolved.gaps){
  ...
  if(!prior){needs.push(need);if(scalar)answer_targets.push({fact_key,input_path:missing.input_path,branch:'pension',index:null,value_kind:type});}
  else if(canonicalSha256(prior)!==canonicalSha256(need))throw Error('ENTITLEMENT_PENSION_QUESTION_COLLISION');
