...
export function resolveTravelEntitlement(candidate:unknown){
 ...
 if(new Set(input.applicability.map(d=>d.decision_id)).size!==input.applicability.length||input.applicability.some(d=>!(d.decision_id in TRAVEL_APPLICABILITY)))throw Error('TRAVEL_DECISION_SET');
 for(const d of input.applicability)for(const s of d.sources){if(s.reading==='source_research'){if(!isPinnedTravelLegalSource(s))throw Error('TRAVEL_LEGAL_SOURCE_PIN');}else bound(s,input);}
 ...
 const finish=()=>deepFreeze({schema_version:'travel-entitlement-resolution-v1' as const,input_sha256:canonicalSha256(input),catalog:TRAVEL_CATALOG,branch,checks,gaps,rule_metadata:{source_review_sha256:TRAVEL_SOURCE_REVIEW_SHA256,cap_is_not_entitlement:true,paid_fare_receipt_required:false,amount_is_not_cash_debt:true,human_attestation:null,real_activation_allowed:false}});
 ...
 for(const compared of [false,true]){
  ...
  const rule=createRuleSpecPackage({schema_version:'tivdoc-rulespec-v0.6.0',rule_spec_id:`il.review.travel.${variant}.${compared?'comparison':'expected'}`,rule_spec_version:'1.0.0',topic:'travel',catalog_boundary:'real_inactive',source_version_ids:TRAVEL_LEGAL_MANIFEST.map(s=>s.version_id),effective_period:TRAVEL_SOURCE_REVIEW.supported_period,sectors:['general_private_conditionally_assessed'],populations:['adult_general'],facts,parameters,nodes,output_ref:compared?'travel.difference':expectedRef,golden_case_set_sha256:canonicalSha256({daily:'12.00',days:20,pass:'200.00',expected:'200.00',legacy_formula_sha256:TRAVEL_ACTUAL_COST_SPEC.content_sha256}),resource_policy:{max_steps:10,max_depth:6,max_aggregate_items:8,max_integer_digits:64}});
  ...
  const prior=input.source_manifest.find(m=>m.document_id===TRAVEL_LEGAL_MANIFEST[0].document_id);if(prior&&canonicalSha256(prior)!==canonicalSha256(TRAVEL_LEGAL_MANIFEST[0]))throw Error('TRAVEL_LEGAL_MANIFEST_PIN');if(!manifest.some(m=>m.document_id===TRAVEL_LEGAL_MANIFEST[0].document_id))manifest.push(TRAVEL_LEGAL_MANIFEST[0]);
  ...
  checks.push({check_id:calculation.check_id,topic:'travel',title:compared?'השתתפות בנסיעות לעומת הסכום שנרשם':'השתתפות צפויה בנסיעות',explanation:zero?'לפי העובדות שזוהו, ענף זה אינו יוצר השתתפות צפויה במסגרת הצו הכללי; הסדר מיטיב ותחולה נבדקים בנפרד.':'סכום מותנה במסלול, ימים, הנחות וכרטיס מתאים; התקרה אינה סכום המגיע אוטומטית. הפער הוא צפוי פחות רשום ואינו קביעה על חוב מזומן.',calculation});
 }
 return finish();
}
