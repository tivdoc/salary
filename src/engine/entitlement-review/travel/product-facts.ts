...
export function travelProductFactQuestions(input:TravelEntitlementInput):TravelProductQuestion[]{
 ...
 return out.filter(q=>!usable(q.fact)&&!(q.path==='product_facts.other_travel_terms_known'&&arrangementCurrent));
}
...
export function evaluateTravelCaseRecipe(decisionId:string,input:TravelEntitlementInput,review?:DocumentReviewInput){
 ...
 const sameGroup=(s:DocumentReviewSource)=>s.document_id===group.document_id&&s.version_id===group.version_id&&s.file_sha256===group.file_sha256&&s.reading_receipt_sha256===group.reading_receipt_sha256&&s.page===group.page;
...
export function travelCaseDecisionSources(input:TravelEntitlementInput,id:string){if(!input.case_recipe_bindings?.some(b=>b.method.recipe_id==='ai-case.'+id))return [];const ready=evaluateTravelCaseRecipe(id,input);return ready.allowed?sourcesIn(ready.consumed_paths.map(p=>at(input,p))):[];}
...
export function replayTravelProductFacts(effective:TravelEntitlementInput,original:TravelEntitlementInput,review?:DocumentReviewInput){
 ...
 for(const b of bindings){travelCaseRecipeBindingSchema.parse(b);const recipe=AI_RELEASE_DECISION_RECIPES.find(r=>r.recipe_id===b.method.recipe_id&&r.recipe_id.startsWith('ai-case.travel.'));
  if(!recipe||recipe.branch!=='travel'||recipe.recipe_sha256!==b.method.recipe_sha256||recipe.recipe_version!==b.method.recipe_version||b.method.source_policy_sha256!==TRAVEL_SOURCE_REVIEW_SHA256||b.method.issued_at>b.evaluated_at||b.method.expires_at<=b.evaluated_at||recipe.legal_sources.some(s=>!b.method.source_receipts.some(r=>r.source_version_id===s.version_id&&r.artifact_sha256===s.file_sha256)))throw Error('TRAVEL_CASE_BINDING_SCOPE');
