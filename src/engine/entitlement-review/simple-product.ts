...
type Gap={key:string;path:string;state:string;kind:'missing_fact'|'missing_source'|'missing_applicability'|'missing_rule';question:string;ids:readonly string[];answer_kind:string;date_format?:'iso_date'|'iso_date_or_ongoing'};
...
export function simpleEntitlementProduct(input:DocumentReviewInput,topic:Topic,candidate:unknown):EntitlementBranchReview{
 ...
 for(const m of missing){
  ...
  const scalar=Boolean(field||boolean||date),legal=['missing_applicability','missing_rule'].includes(m.kind);
  const need:ReviewCompletionNeed={fact_key,kind:legal?'legal':'factual',reason:m.state==='conflict'?'conflicted':m.state==='unreadable'?'unreadable':m.state==='missing'?'missing':'unknown',
   ...
   answer_kind:field?'choice':boolean?'boolean':m.answer_kind==='number'&&!legal?'number':'text',
   ...
   ...(date?{value_validation:{schema_version:'document-review-value-validation-v1' as const,format:date}}:{}),source_pins:pins,dependent_check_ids:[...m.ids],general_question:false};
  if(!needs.some(n=>n.fact_key===fact_key)){
   needs.push(need);if(scalar)answer_targets.push({fact_key,input_path:m.path,branch:topic,index:null,value_kind:boolean?'boolean':date==='iso_date'?'date':date?'date_or_ongoing':'text'});
  }
