...
import { selectCanonicalRetrievalChunks } from "./corpus-hardening/canonical-corpus.ts";
...
export function retrieveLegalKnowledgeCore(
  ...
  const canonicalChunks = selectCanonicalRetrievalChunks(eligibleSources, chunks);
  const results = canonicalChunks.flatMap((chunk): LegalKnowledgeResult[] => {
