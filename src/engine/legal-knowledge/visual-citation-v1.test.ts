...
const BASE = {
  ...
  line_index: 38,
  line_text: "ןהירחאש תפסונ העש לכ דעבו ,ליגרה רכשהמ 11/4­מתוחפ אל הדובע רכש םוי ותואבש",
...
describe("legal-visual-citation-v1 (L6-2, D1)", () => {
  ...
  it("builds a citation that says what it is: inferred, visual, awaiting confirmation", () => {
    ...
    expect(citation.region).toEqual({ kind: "stored_line", line_index: 38, line_text: BASE.line_text });
  ...
  it("refuses a surface that is not on the stored line, and one the lexicon would read instead", () => {
    ...
    expect(buildVisualCitation({ ...BASE, line_text: "שכר עבודה לא פחות מ-מחצית", text_layer_surface: "מחצית" }).refusal).toBe("VISUAL_SURFACE_NOT_AMBIGUOUS_USE_TEXT_PATH");
    expect(buildVisualCitation({ ...BASE, line_text: "   " }).refusal).toBe("VISUAL_STORED_LINE_EMPTY");
