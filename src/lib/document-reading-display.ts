...
export type DocumentReadingRequestGroup<T>=Readonly<{kind:'single';requests:readonly[T]}|{kind:'row';requests:readonly T[];group_id:string;label:string;page:number}>;
...
export function groupDocumentReadingRequests<T extends ReadingRequest>(requests:readonly T[]):readonly DocumentReadingRequestGroup<T>[] {
 const eligible=(r:T)=>r.source_current===true&&r.reading_display?.row_context
  &&/^[a-f0-9]{64}$/u.test(r.reading_display.row_context.group_id)?r.reading_display.row_context.group_id:null;
 ...
 const valid=new Set([...buckets].filter(([,rows])=>rows.length>1
  &&new Set(rows.map(r=>r.reading_display!.row_context!.cell)).size===rows.length
  &&new Set(rows.map(r=>r.reading_display!.row_context!.label)).size===1
 ...
 for(const request of requests){const key=eligible(request);
  ...
  result.push({kind:'row',requests:buckets.get(key)!,group_id:key,label:request.reading_display!.row_context!.label,page:request.reading_display!.page});
 }
 return result;
}
