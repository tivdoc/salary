## Source structures integrated, verified and stopped — 2026-09-12

[Current handoff](source-structures-handoff-2026-09-12-he.md) and [nine-topic completion map](service-nine-topics-completion-map-2026-09-12-he.md). Application **f64fdbf68da0646f992b795ee6011c0b0ee3fb7f**, isolated DEV **schema 170**, CI [34659335758](https://github.com/tivdoc/salary/actions/runs/34659335758) / [34659333456](https://github.com/tivdoc/salary/actions/runs/34659333456) both succeeded. Source relationship, deduction inventory/grouping and five balance-cell decisions are integrated into the existing identified-reading and ordinary report path; old receipts and answers are preserved.

[Preview](https://salary-e2u76evyv-tivdoccom-5042s-projects.vercel.app) deployment 6403658385 succeeded at the same SHA; GET is 302 to Vercel SSO. Authenticated verification is local/isolated DEV, not an authenticated Preview session. Documentation commits are separate from the tested runtime.

The retained live source now has **13 checks, 10 calculated**, at source revision 77. Two missing fund/base links and blank overtime quantity/amount remain explicit. The 76→77 group-basis refinement does not change any mathematical result. A separately uploaded marked synthetic QA source, injected extraction and actual browser relationship/number decisions produce **5% observed ratio** through the normal worker; no new OCR or required pension rate is claimed.

The five protected owner drafts total **23 pages**, with 8 + 5 independent comparisons passing and four other drafts retaining their exact prior bytes. Final revision-77 Chrome PDF/HTML hashes match DB; current/foreign/anonymous access, old-report 410 and fresh identical-manifest retry passed. Two real worker connections passed 13 contract/concurrency checks; the nonexistent-version negative is not an old-file replacement proof. The separate synthetic browser sequence passed 55 DB assertions for correction/unreadable/restoration with independent numeric receipts preserved. Actual new-PDF replacement passed 14 DB assertions plus old-source/report/answer 404/410/409; both versions remain and no new extraction/analysis was performed after replacement.

**STOP verified:** synthetic state stopped, two scoped package machines revoked and six matching configurations disabled; others unchanged. The matching local application remains available for viewing with automatic worker/outbox/provider disabled and unchanged ledger hash. No additional provider content calls; ledger unchanged at 10/12 requests and USD 3.56 reserved/5 with two historical unknown costs. Full nine-topic entitlement and REAL activation remain separate unfinished work. Older entries below are historical.

## Exact component readings, ordinary reports and period coverage — 2026-09-12

[Current handoff](component-cell-readings-handoff-2026-09-11-he.md), [redacted verification](release-evidence/component-cell-review-20260911/verification.json). Tested application7ec25d7e8254e7a6e20aaecd7e0c3d42a3966851, DEVschema169; CI34649733194/34649728906 bothPASS (4,973 tests/103 configured skips, typecheck/lint/build/48-check closure). Source-bound row amount/quantity/rate/percentage contracts, identified reading history, targeted grouped questions, safe source transcription and preserved historical topic/month coverage are integrated into the normal saved analysis/report path.

CASE-07 uses the retained exact live receipt and normal browser answers:7/10 calculated,3 blocked; three revisions and165 independent assertions prove dependency-local changes and history. CASE-04:22 June checks/19 calculated,24 excluded checks explicitly mapped,9 topics retained, purchase period not invented. Final full local application/DEV source, retry, currentness, authorization and actual browser HTML/PDF downloads PASS;74 amounts and12 pages verified. Five separate private curated comparison drafts updated,11 oracle assertions/25-page visual review PASS. Preview is built but SSO-blocked; local3420 is the verified fallback. Private owner index: release-work/row-cell-flow-20260911/owner-delivery.private.html.

Remaining code for source relationships, deduction-group admission and balance movement readings is explicitly separate from missing customer data and REAL activation decisions. No fresh provider calls; unchanged10/12 requests,USD3.56reserved/5,2unknown historical costs. Two scoped machines revoked,three configurations paused; viewing app remains, processing/outbox off. No customer delivery, charges, Production or parallel UI changes. Later documentation commits do not change the tested application. Package delivered with the stated limits; no automatic next package.

## Document review integrated into the regular product — 2026-09-11

See [current product/verification handoff](document-review-product-handoff-2026-09-11-he.md) and [evidence matrix](release-evidence/document-review-20260911/verification.json). Application **3ccb33b5f0c7872167d1f22213f011c59131cca8**, both CI34616077301/34616072546 PASS: **4,502 tests passed, 102 configured skips**, typecheck/lint/build and 48 Production-closure checks. Isolated DEV schema **159** passed ten actual worker/web DB checks on clean e54b3c6; later import/test corrections do not change SQL or calculation behavior.
...
Actual original100/120 retained; hours100 are an identified synthetic declaration, not silently selected OCR. Complex-PDF source labels pass the same oracle onr7. One canonical result240.58 conditional on isolated synthetic authority/payment; REAL refuses missing genuine approvals. Budget6/12 content requests and2.136/5USD reserved, unknown0. STOP02:27:44UTC disabled scheduler/supervisor/package and revoked scoped machine/capability/enrollment; private configurations and history remain. Live webhook ingress, a new natural-expiry observation and a post-STOP renewal command remain gaps. Production/parallel UI unchanged; no automatic continuation.
...
Internal branch-Preview encryption/owner allowlist/DEV webhook DSN and private worker overlay are prepared; actual OpenAI/Resend account access is blocked. Zero live extraction/send/delivery/webhook calls and no live financial report. Eleven-input corpus includes synthetic Hebrew and raster,60focused tests; canonical June sources/components/parameters/pure executor and unsigned review packet are saved. Customer declarations now reach the same canonical review stage, but admitted evidence and ordinary executor binding remain open; see the exact [interface boundary](june2026-canonical-binding-boundary.md). Scheduler and provider flags remain disabled. Existing salary Production app/DB were untouched; two misclassified first deployments of a separate empty ingress project were deleted immediately. Public ingress is not deployed. Delivery afterf0d8081 changes docs/evidence only. STOP; do not resume historical P00–P13 without a new task.

Prior [automatic DEV proof](automatic-dev-flow-handoff-2026-09-09-he.md) remains separately attributed to71be722/schema127:11DB/Storage,5notification,11hosted plus6replacement and8OTP-service checks, with injected OCR/mail/payment/customer-session fixtures. Earlier [financial-flow proof](dev-financial-flow-handoff-2026-09-09-he.md) is also retained. Neither proves the newly requested live providers.
