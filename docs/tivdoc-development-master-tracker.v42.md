...
הערת שחזור: קובץ זה נוצר מחדש בתוך המאגר מגרסה 36 של הבעלים, בקריאה בלבד, עם החלת דלתאות v37–v41 ודלתא v42 של ריצה 7. הקובץ המקורי של הבעלים לא נערך.
...
|---:|---|---|---|---|
| 1.1 | Engine Foundation | ✅ | חוזי domain מרכזיים קיימים ומחוברים; מודל העובדות ברשימה אחת (30 נתיבים), סכימת ערך לכל נתיב | — |
...
| 1.5 | Deterministic Rule Runtime | 🟠 | R-1…R-14 סגורים; ‏14 סוגי צמתים; רישום יחידות דו־כיווני; ‏13 מפרטי טיוטה בני־ביצוע, ‏7/7 נושאים רצים בדוח הרגישות v6; עקבות R-14 משוחזרות בית־בבית | RuleSpecs מאושרים על ידי אדם |
...
| 1.8 | Offline Shadow Mode | 🟡 | מעטפה v0.11 במצב `draft_parameters_synthetic_inputs`: ‏54 חודשי תלוש סינתטיים × 13 מפרטי טיוטה, ‏123 הרצות, ‏86 רצו, ‏86 עקבות שוחזרו; הפרש־צל סינתטי לכל רכיב תשלום; ‏0/7 real topics | פרמטר וכלל מאושרים; אז Offline legal shadow |
| 1.9 | Customer Shadow Mode | ⛔ | אסור | Offline Shadow מוצלח ואישור מפורש |
| 1.10 | Production Legal Engine | ⛔ | מחוץ לתחום | Shadow, אבטחה, פרטיות ואימות משפטי מלא |
...
| 1.13 | Ledger Truthfulness | 🟡 | ‏60 sweep ids מפויסים; רישום כותבי governance מלא (כל סקריפט כותב נקוב עם ה־tenant שלו) | H-4 |
...
|---:|---|---|---|---|
| 2.1 | Branch ייעודי לפיתוח הליבה | ✅ | `claude/v0-10-2b-full-parallel` | לשמור branch מוגן ונקי |
...
| 2.12 | Production/deploy prohibition | ✅ | ‏0 בכל הריצות | להמשיך |
...
| 2.20 | רישום כותבי governance | ✅ | בדיקה מונה כל סקריפט שכותב מצב governance ואת ה־tenant שלו; סקריפט חדש שאינו רשום מפיל את הבדיקה | לשמר |
...
|---:|---|---|---|---|
| 11.1–11.5 | Shadow control-plane, envelope, kill switch, offline synthetic | 🟡 | מעטפה v0.11 (‏v0.10 עדיין תקפה); מצב `draft_parameters_synthetic_inputs` עם נעיצה (0 פרמטרים פעילים, 28 גרסאות טיוטה, 54 קלטים סינתטיים, חילוץ לא בשימוש); מתזמן עמיד עם lease מגודר ושרשרת ביקורת; מתג חירום כבוי כברירת מחדל ומוכח; ‏0/7 real | RuleSpec/parameter מאושרים |
| 11.6 | Offline legal shadow | ⛔ | אסור על פרמטרים אמיתיים; ריצת הצל על טיוטות: ‏86 מקרים רצו, ‏61 הפרשי־צל, ‏0 ממצאים, ‏0 מסירות | first approved rule |
| 11.7 | Customer Shadow Mode | ⛔ | אסור | GT + privacy authorization |
...
| 11.10–11.11 | Production readiness / activation | ⛔ | אסור | החלטה מפורשת נפרדת |
...
| BL-25 | ‏7 גרסאות inferred_visual ממתינות לאימות חזותי אנושי | 🔴 Open | קריטית משפטית | Legal reviewer | `visual_confirmed: true` מול העמודים שבחבילה v11 |
...
RULESPEC_DRAFTS_7_EVERY_SLOT_BOUND_EXECUTABLE_SPECS_13
SENSITIVITY_REPORT_V6_TOPICS_7_OF_7_SCENARIOS_102_RUN_85_TRACES_85_REPLAYED_85
RULE_INPUT_MAPPING_REGISTRY_V2_SLOTS_17_OF_17_TRANSFORMATIONS_12_REJECTION_CODES_9_PROVEN
OFFLINE_SHADOW_ENVELOPE_V011_MODE_DRAFT_PARAMETERS_SYNTHETIC_INPUTS
OFFLINE_SHADOW_DRAFT_RUN_CASES_54_EXECUTIONS_123_RAN_86_TRACES_86_REPLAYED_86
SYNTHETIC_SHADOW_DELTAS_61_COMPUTED_25_NOT_APPLICABLE_0_FINDINGS
...
HEBREW_SENSITIVITY_REPORT_V6_MARKDOWN_AND_PDF_HASH_BOUND
REVIEW_PACKAGE_V11_FILES_32_DETERMINISTIC_TOPICS_RUN_GATE_7_SHADOW_CASES_RUN_GATE_86
TRACKER_V42_REGENERATED_IN_REPOSITORY_OWNER_FILE_UNTOUCHED
EXTRACTION_USED_NO
...
הערה: שבע ריצות רציפות ו־Addendum 4 סגרו את כל ההנדסה שלא צריכה אדם: ‏59 גרסאות פרמטר בטיוטה מציטוטים אמיתיים בארבעה סוגים, ‏7 טיוטות RuleSpec שכל משבצת בהן קשורה, ‏13 מפרטים בני־ביצוע, דוח רגישות שרץ ‏7/7 נושאים ומדרג כל מקור, והצל הלא־מקוון שמריץ את הטיוטות על חודשי תלוש סינתטיים דרך מודל העובדות של המוצר עצמו — כל פלט הפרש־צל סינתטי, לא ממצא. מה שנותר צריך עורך דין, זהות reviewer אמיתית או מקור רשמי שאינו ניתן לאיתור: ביקורת הדוח, אימות חזותי של שבע קריאות, שתי attestations, ואישור RuleSpec. ‏0/7 נושאים, ‏0 מקורות, ‏0 פרמטרים, ‏0 כללים, ‏0 חישובים ו־0 Human GT נשארים אמת מחייבת — ומעכשיו הם משתנים רק בידי אדם.
