-- L6-2 / D1. Visual citations reach attestation.
--
-- A figure read from a page image (legal-visual-citation-v1) is registered
-- with provenance_grade = inferred_visual and a visual_bindings list — for
-- each visual citation, the hash of the cited page extracted as a standalone
-- PDF and the reading the session made. This replaces
-- governance_parameter_attestation_append (last defined in 202609020020) with
-- one more refusal in the same place as the binding checks: such a candidate
-- is attested only with visual_confirmed = true and the identical bindings,
-- and a candidate without visual bindings refuses an attestation that claims
-- to confirm any. Everything else in the function is byte-for-byte the
-- previous definition, including the decision cascade.
...
begin
  ...
  -- L6-2 / D1. A candidate whose figure was read from the page image rather
  -- than from the text (provenance_grade = inferred_visual) may only be
  -- attested by someone who says they looked: visual_confirmed must be true,
  -- and the attestation must name the very page hashes and readings the
  -- candidate carries — nothing that could be copied from another page.
  -- A candidate without visual bindings may not be attested with them either;
  -- a confirmation of nothing is not a confirmation.
  if current_version.candidate_json ->> 'provenance_grade' = 'inferred_visual'
     or pg_catalog.jsonb_typeof(current_version.candidate_json -> 'visual_bindings') = 'array' then
    if target_attestation -> 'visual_confirmed' is distinct from 'true'::jsonb
       or pg_catalog.jsonb_typeof(current_version.candidate_json -> 'visual_bindings') is distinct from 'array'
       or pg_catalog.jsonb_array_length(current_version.candidate_json -> 'visual_bindings') = 0
       or target_attestation -> 'visual_bindings' is distinct from current_version.candidate_json -> 'visual_bindings' then
      raise exception using errcode = 'P0001', message = 'GOVERNANCE_PARAMETER_VISUAL_CONFIRMATION_REQUIRED';
    end if;
  elsif target_attestation ? 'visual_confirmed' or target_attestation ? 'visual_bindings' then
    raise exception using errcode = 'P0001', message = 'GOVERNANCE_PARAMETER_VISUAL_CONFIRMATION_NOT_APPLICABLE';
  end if;
