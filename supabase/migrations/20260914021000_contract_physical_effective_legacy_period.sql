-- Additive215: admit authenticated effective legacy source periods for physical
-- contract discovery. Original receipt periods and all211 identity/source/lease
-- guards remain intact. No new authority, grants, OCR or business receipt edits.
...
begin
 ...
 select exists(
  ...
  select 1 from jsonb_array_elements(coalesce(journal->'legacy_orders','[]'::jsonb)) o
   -- Missing receipt periods may be resolved only by the existing authenticated
   -- current payslip/attendance reading helper. Contracts cannot supply a month.
   cross join lateral jsonb_array_elements(coalesce(o->'periods','[]'::jsonb) ||
    case when o->>'period_state'='missing'
     then coalesce(private.legacy_source_period_evidence(target_case,o)->'periods','[]'::jsonb)
     else '[]'::jsonb end) p
   cross join lateral (select (p#>>'{period,from}')::date first_day,(p#>>'{period,to}')::date last_day) bounds
   cross join lateral (select case when bounds.first_day=date_trunc('month',bounds.first_day)::date
    then bounds.first_day else (date_trunc('month',bounds.first_day)+interval '1 month')::date end month_start) complete
   where coalesce(o->'topics' ?| array['contract','bonuses'],false)
    and complete.month_start+interval '1 month'-interval '1 day'<=bounds.last_day
    and private.document_review_paid_scope_current(target_case,(o->>'id')::uuid,'legacy_paid_receipt',o->>'receipt_sha256',o->'topics',complete.month_start)
 ) into paid;
