-- Operational collection policy v2, not a legal dependency/entitlement rule.
-- Seven shared payslip readings support document consistency. Dedicated cells
-- are requested only for their purchased topic. No historical row is rewritten.
create function private.document_field_question_fields_v2(purchased_topics text[]) returns text[]
 language sql immutable security invoker set search_path='' as $$
 select coalesce(array_agg(f.field order by f.field),'{}'::text[]) from (values
  ('salary_type',null::text),('salary_period',null),('base_monthly_salary',null),('hourly_rate',null),('gross_salary',null),('net_salary',null),('regular_hours',null),
...
$$;
revoke all on function private.document_field_question_fields_v2(text[]) from public,anon,authenticated,service_role,tivdoc_web_runtime,tivdoc_operations_runtime;
grant execute on function private.document_field_question_fields_v2(text[]) to tivdoc_worker_runtime;
...
begin
 ...
 if not coalesce(target_payload->>'schema_version'='document-field-confirmation-v1' and target_payload->>'case_id'=target_case::text
  ...
  and candidate_field in ('salary_type','salary_period','base_monthly_salary','hourly_rate','gross_salary','net_salary','regular_hours','overtime_125_hours','overtime_150_hours','pension_base','travel_amount','convalescence_amount','vacation_balance','sick_balance'),false)
 ...
 if not exists(select 1 from private.case_input_versions v
  ...
  where v.case_id=target_case and v.revision=expected_revision and v.input_sha256=expected_input_sha256
   ...
   and candidate_field=any(private.document_field_question_fields_v2(o.topics))) then raise exception 'REQUEST_FIELD_UNPURCHASED_TOPIC';end if;
