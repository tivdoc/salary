-- Reviewed canonical source for the installed forward migration at
-- supabase/migrations/202609010001_controlled_import_ledger.sql.
-- The isolated canonical PostgreSQL verifier proves this contract dynamically.
...
create index if not exists controlled_import_requests_recovery_idx
  on private.controlled_import_requests(state, lease_expires_at, created_at, operation_id)
  where state in ('received', 'leased', 'validated');
create index if not exists controlled_import_audit_operation_sequence_idx
  on private.controlled_import_audit_events(operation_id, sequence);
...
begin
  ...
  if exists (
    ...
    where r.operation_id = p_operation_id and r.state = 'published'
      and p.publication_id = p_publication_id
      and p.artifact_sha256 = p_artifact_sha256
  ) then
    return query
      select * from public.controlled_import_publication_status_v1
      where operation_id = p_operation_id;
    return;
  end if;
  if exists (
    ...
    select 1 from public.controlled_import_publication_markers p
    where p.publication_id = p_publication_id and p.operation_id <> p_operation_id
  ) then
    raise exception using errcode = 'CI001', message = 'CONTROLLED_IMPORT_PUBLICATION_REPLAY_MISMATCH';
  end if;
  if not exists (
    select 1 from private.controlled_import_requests r
...
revoke all on function private.append_controlled_import_audit(text,text,jsonb,timestamptz) from public, anon, authenticated, service_role;
revoke all on function private.controlled_import_forbid_mutation() from public, anon, authenticated, service_role;

alter table private.controlled_import_requests enable row level security;
alter table private.controlled_import_artifacts enable row level security;
alter table private.controlled_import_audit_events enable row level security;
alter table public.controlled_import_publication_markers enable row level security;

grant usage on schema private to service_role;
grant select on public.controlled_import_publication_status_v1 to service_role;
grant execute on function private.controlled_import_reserve(text,text,text,text,jsonb,text,text,timestamptz) to service_role;
grant execute on function private.claim_controlled_import_recovery(text,timestamptz,interval,integer) to service_role;
grant execute on function private.controlled_import_stage_exact_bytes(text,text,integer,bytea,text,text,timestamptz) to service_role;
grant execute on function private.controlled_import_reject(text,text,integer,text,timestamptz) to service_role;
grant execute on function private.controlled_import_publish(text,text,integer,text,text,timestamptz) to service_role;
grant execute on function private.open_controlled_import_published_bytes(text) to service_role;

insert into public.engine_schema_metadata (component, schema_version, migration_id)
values (
  'controlled_import_ledger',
  'tivdoc-controlled-import-ledger-v0.10.0',
  '202609010001_controlled_import_ledger'
)
