...
const bundledPython = process.env.USERPROFILE ? path.join(process.env.USERPROFILE, ".cache", "codex-runtimes", "codex-primary-runtime", "dependencies", "python", "python.exe") : "";
const python = bundledPython && existsSync(bundledPython) ? { command: bundledPython, prefix: [] as string[] } : { command: "py", prefix: ["-3"] };
...
type CommandDefinition = Readonly<{ id: string; command: string; args: readonly string[]; expectedExit: number }>;
...
const commands: readonly CommandDefinition[] = [
  ...
  npmRun("legal-changes", "legal:sources:changes"),
  npmRun("legal-catalogs", "legal:sources:catalogs"),
  ...
  npmRun("corpus-persistent-readiness", "legal:sources:readiness", 1, ["--from", "2019-01-01", "--as-of", "2026-08-30", "--sector", "general"]),
  ...
  npmRun("wave21-real-reader-crash-matrix", "wave21:controlled-import:local"),
