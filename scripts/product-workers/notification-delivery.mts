...
import {deliverNotificationOutbox,enqueueRequestReminders} from '../../src/server/product/case-access/notification-outbox.ts';
...
try{const store=postgresCaseAccessDb(connection);if(process.env.TIVDOC_REQUEST_REMINDER_DELIVERY_ENABLED==='true'){const origin=process.env.NEXT_PUBLIC_SITE_URL;if(!origin)throw new Error('NOTIFICATION_ORIGIN_MISSING');await enqueueRequestReminders(store,secret,origin);}const workerId=randomUUID();let attempted=0;for(;attempted<20;attempted++){const result=await deliverNotificationOutbox(store,workerId,secret,resendProvider(apiKey,from));if(!result)break;}console.log(JSON.stringify({worker:'notification_delivery',attempted}));}finally{await connection.end();}
