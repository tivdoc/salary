...
def safe_name(name: str) -> pathlib.PurePosixPath:
    ...
    for part in value.parts:
        ...
        if part in ("", ".") or part != part.rstrip(" .") or stem in WINDOWS_DEVICES:
...
def verify_and_extract(archive_path: pathlib.Path, expected: dict[str, dict[str, object]]) -> int:
    with tempfile.TemporaryDirectory(prefix="tivdoc-wave21-consumer-") as temporary:
...
def main() -> int:
    if len(sys.argv) != 3:
        raise SystemExit("usage: parallel-wave21-review-package-zip.py PACKAGE_DIR OUTPUT_ZIP")
    ...
    print(json.dumps({
        ...
        "rejected_member_classes": ["traversal", "duplicate", "case_collision", "link", "device", "extra", "missing", "changed_bytes"],
    }, sort_keys=True))
    return 0
