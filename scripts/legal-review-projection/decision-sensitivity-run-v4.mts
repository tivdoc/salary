...
async function main(): Promise<void> {
    ...
    for (const [branch, version] of branches) {
      ...
      for (const fixture of fixtures.filter((item) => item.topic === entry.spec.topic)) {
        ...
        if (traceWire) {
          ...
          executionId = `l510.${entry.spec.topic}.${specSlug}.${fixture.scenario}.${branch}.${runSuffix}`.replaceAll("_", "-");
          ...
          await tx("e37_append", (client) => client.query(statement("e37_append_call",
            ...
            [SYNTHETIC_PROOF_TENANT, JSON.stringify(payload), `l510.${executionId}`, sha256(`l510:${executionId}:${traceSha}`), new Date().toISOString()])), PROOF_SESSION);
  ...
  const sensitivity = SENSITIVITY_SPECS.filter((entry) => entry.decision_id !== null).map((entry) => {
    ...
    return {
      ...
      branch_note: BRANCH_NOTES[entry.decision_id] ?? null,
      summary: differing.length === 0 ? (BRANCH_NOTES[entry.decision_id] ?? "no scenario in this set separates the branches")
  ...
  const SHORTFALL: Readonly<Record<string, Readonly<{ slots: readonly string[]; not_run: string; detail: string }>>> = {
    working_time: {
      ...
      detail: "il.working_time.overtime_rate_first_tier is unbound. The only corpus text of the Hours of Work and Rest Law is the 1951 gazette scan, whose OCR renders 1¼ and 1½ as 11/4 and 11/2 — ambiguous, and refused by name by the numeral lexicon rather than read. The 2018 general permit's caps (12 hours a day, 16 overtime hours a week, 58 hours a week) are registered from their instrument selection, but they are ceilings, not rates; the tiered.rate node is built and waiting on a consolidated text. That text is requested through the controlled acquisition path (L5-8); if it lands, §16(א) binds through the lexicon and this topic runs.",
    },
  };
  ...
  const content = {
    schema_version: "tivdoc-decision-sensitivity-report-v4-v0.10.17",
    ...
    replaces: "tivdoc-decision-sensitivity-report-v3-v0.10.16 (v3 ran 4 topics of 7)",
    scope_note:
  ...
  writeFileSync(path.join(RECEIPT_ROOT, "decision-sensitivity-report-v4.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
