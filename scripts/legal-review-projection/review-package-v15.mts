// L13-5 / E1-E7 (long run 10). Review package v15: v14's package, on report
// v10, carrying errata #1 as the legal basis it is.
//
// What is new in v15, and only that. The package's manifest and its report
// member name `legal_basis: opinion_3ddad7e8 + errata_1_owner_closed` — the
// approved opinion AS CORRECTED by errata the owner closed without a lawyer
// (approval record §7), which is what the engine actually runs on. The
// resolutions member carries the two re-recordings (the daily threshold's
// conditional scenario and the pension wage-cap source downgrade) with the
// revision each supersedes, and the superseded revisions travel beside them
// unedited. The Q4 base rule is a registered draft parameter now, bound to
// §18's own words in the corpus, so it appears in the parameters member
// rather than only in prose.
//
// Unchanged: the approved opinion file is not edited and its hash stands, no
// source is reviewed, no parameter leaves draft, nothing is attested, and the
// counters stay zero.
//
// v14 is kept beside v15. (v14's header follows.)
...
import { OWNER_RECORDED_RESOLUTIONS, REJECTED_BRANCHES, resolutionSha256, RESOLUTION_HISTORY } from "../../src/engine/legal-quality/decision-resolutions.ts";
...
const PACKAGE_ROOT = path.join("output", "next", "review-package-v15");
...
async function build(): Promise<{ manifest_sha256: string; files: Written[]; item_count: number }> {
  ...
  const sensitivity = readIfPresent("output/next/pool-q/decision-sensitivity-report-v10.json");
  ...
  const factory = NodePostgresConnectionFactory.fromConnectionUrl({
    ...
    application_name: "tivdoc_review_package_v15",
    remote_dev_target: {
  ...
  // L13-5: append-only by revision — the registry names the latest, the earlier ones are history.
  ...
  if (resolutionMismatches.length > 0 || resolutionRows.length !== OWNER_RECORDED_RESOLUTIONS.length + RESOLUTION_HISTORY.length) throw new Error(`E117_RESOLUTIONS_MISMATCH:${resolutionMismatches.join(",")}:rows_${resolutionRows.length}`);
