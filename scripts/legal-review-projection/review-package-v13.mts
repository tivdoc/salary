...
const PACKAGE_ROOT = path.join("output", "next", "review-package-v13");
...
async function build(): Promise<{ manifest_sha256: string; files: Written[]; item_count: number }> {
  ...
  const sensitivity = readIfPresent("output/next/pool-q/decision-sensitivity-report-v8.json");
  if (!sensitivity) throw new Error("E117_SENSITIVITY_REPORT_V8_MISSING:run decision-sensitivity-run-v8.mts first");
  ...
  const factory = NodePostgresConnectionFactory.fromConnectionUrl({
    ...
    application_name: "tivdoc_review_package_v13",
    remote_dev_target: {
  ...
  try {
    await client.query(statement("v13_begin", "begin", []));
    await client.query(statement("v13_context", "select * from private.runtime_context_install($1,$2,$3)",
      [SYSTEM_SESSION.sid, SYSTEM_SESSION.jti, `v12:${randomUUID().slice(0, 8)}`]));
    const decisions = await client.query(statement("v13_decisions",
    ...
    const resolutionRead = await client.query(statement("v13_resolutions",
    ...
    for (const id of [...wanted].sort()) {
      ...
      const row = await client.query(statement("v13_aggregate",
    ...
    await client.query(statement("v13_rollback", "rollback", []));
  } finally {
    client.release();
  }

  writeMember(files, "decisions/legal-open-decisions.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-decisions",
  ...
  const previousManifestForResolutions = readIfPresent("output/next/review-package-v12/manifest.json");
  ...
  writeMember(files, "decisions/legal-decision-resolutions.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-resolutions",
  ...
  writeMember(files, "decisions/branches-examined-and-rejected.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-rejected-branches",
  ...
  writeMember(files, "evidence/owner-evidence-index.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-owner-evidence",
  ...
  writeMember(files, "decisions/open-items-v1-v13.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-open-items",
  ...
  for (const receiptName of ["batch-17-average-wage.json", "batch-18-havraa-year.json", "batch-19-rest-day-daily-threshold.json"]) {
  ...
  writeMember(files, "parameters/draft-parameters.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-parameters",
  ...
  writeMember(files, "rulespecs/templates.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-rulespec-templates",
    templates: buildSevenRuleSpecTemplates(),
  }));
  writeMember(files, "rulespecs/executable-sensitivity-specs.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-sensitivity-specs",
  ...
  writeMember(files, "rulespecs/drafts.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-rulespec-drafts",
  ...
  const previousManifest = readIfPresent("output/next/review-package-v12/manifest.json");
  const previousTopicsMember = readIfPresent("output/next/review-package-v12/scenarios/topics-not-run.json");
  ...
  writeMember(files, "scenarios/scenario-input-fixtures.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-scenario-fixtures",
  ...
  writeMember(files, "scenarios/executions-and-traces.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-executions",
  ...
  writeMember(files, "scenarios/topics-not-run.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-topics-not-run",
  ...
  writeMember(files, "shadow/branch-comparison.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-branch-comparison",
  ...
  writeMember(files, "shadow/synthetic-corpus-index.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-synthetic-corpus-index",
  ...
  writeMember(files, "visual-pages/index.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-visual-pages",
  ...
  writeMember(files, "parameters/citation-anchors.json", canonicalJson({
    schema_version: "tivdoc-review-package-v13-citation-anchors",
  ...
  const manifestContent = {
    schema_version: "tivdoc-review-package-v13-manifest",
    package: "review-package-v13",
    tenant: TENANT,
    ...STATE,
