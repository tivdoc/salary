...
const BRANCH_NOTES: Readonly<Record<string, string>> = Object.freeze({
  [`${TENANT}.decision.working_time_daily_threshold`]:
    "only the statute branch (eight hours, §2(א) of the 1951 law, bound through the lexicon) runs; the administrative branch (8.6 hours on a five-day week / 7.6 on a six-day week, Labour Ministry directive of 10.6.2018) is unbound — BL-24, its official text not discoverable, a mirror copy not acceptable — so no scenario can separate the branches here, and the report says so rather than showing one column as agreement",
...
async function main(): Promise<void> {
    ...
    for (const [branch, version] of branches) {
      ...
      for (const fixture of fixtures.filter((item) => item.topic === entry.spec.topic)) {
        ...
        if (traceWire) {
          ...
          executionId = `l710.${entry.spec.topic}.${specSlug}.${fixture.scenario}.${branch}.${runSuffix}`.replaceAll("_", "-");
          ...
          await tx("e37_append", (client) => client.query(statement("e37_append_call",
            ...
            [SYNTHETIC_PROOF_TENANT, JSON.stringify(payload), `l710.${executionId}`, sha256(`l710:${executionId}:${traceSha}`), new Date().toISOString()])), PROOF_SESSION);
  ...
  const shadowReceipt = JSON.parse(readFileSync(shadowReceiptPath, "utf8")) as {
    ...
    comparison: Array<{ decision_id: string; branches: string[]; unbound_branches?: unknown[]; cases_compared: number; cases_differing: number; cases_not_comparable: number; human_review_required: boolean; automatic_acceptance: boolean }>;
  ...
  const content = {
    schema_version: "tivdoc-decision-sensitivity-report-v7-v0.12.0",
    ...
    replaces: "tivdoc-decision-sensitivity-report-v6-v0.11.0 (v6 ran the shadow on thirteen specs with the population as a label; v7 runs fifteen with the population as a fact)",
    scope_note:
  ...
  writeFileSync(path.join(RECEIPT_ROOT, "decision-sensitivity-report-v7.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
