// E2-1. Review package v6: everything a lawyer needs, in one place, built twice
// to one hash.
//
...
//
// One thing the brief asked for that is not in here, because it does not exist:
// **the 42 scenario traces per branch.** The draft specs cannot execute while
// their judgement slots are unbound and the golden cases are blank templates,
// so there are no traces. What is included instead is the record of all 42
// scenarios attempted and refused, each with both specific reasons. A package
// that shipped an empty `traces/` directory would read as an oversight; this
...
const PACKAGE_ROOT = path.join("output", "next", "review-package-v6");
...
async function build(): Promise<{ manifest_sha256: string; files: Written[]; item_count: number }> {
  ...
  const sensitivity = readIfPresent("output/next/pool-q/decision-sensitivity-report.json");
  if (!sensitivity) throw new Error("E21_SENSITIVITY_REPORT_MISSING:run decision-sensitivity-report.mts first");
  ...
  const factory = NodePostgresConnectionFactory.fromConnectionUrl({
    ...
    application_name: "tivdoc_review_package_v6",
    remote_dev_target: {
  ...
  try {
    await client.query(statement("v6_begin", "begin", []));
    await client.query(statement("v6_context", "select * from private.runtime_context_install($1,$2,$3)",
      [SYSTEM_SESSION.sid, SYSTEM_SESSION.jti, `v6:${randomUUID().slice(0, 8)}`]));
    const decisions = await client.query(statement("v6_decisions",
      "select * from private.legal_open_decision_read($1)", [TENANT]));
    decisionRows = decisions.rows as unknown as Array<Record<string, unknown>>;
    ...
    for (const id of [...wanted].sort()) {
      ...
      const row = await client.query(statement("v6_aggregate",
    ...
    await client.query(statement("v6_rollback", "rollback", []));
  } finally {
    client.release();
  }

  writeMember(files, "decisions/legal-open-decisions.json", canonicalJson({
    schema_version: "tivdoc-review-package-v6-decisions",
    ...
    note: "Withdrawn is not resolved. A withdrawn decision was dissolved by reading the source and carries its reason and the citation locator that dissolved it; a resolved one was settled by two independent human attestations naming the same branch. Both are listed; neither is inferred from the other.",
    decisions: decisionRows,
  }));

  writeMember(files, "parameters/draft-parameters.json", canonicalJson({
    schema_version: "tivdoc-review-package-v6-parameters",
  ...
  writeMember(files, "rulespecs/templates.json", canonicalJson({
    schema_version: "tivdoc-review-package-v6-rulespec-templates",
    templates: buildSevenRuleSpecTemplates(),
  }));
  writeMember(files, "rulespecs/drafts.json", canonicalJson({
    schema_version: "tivdoc-review-package-v6-rulespec-drafts",
  ...
  // --- The scenarios: attempted, not run, with reasons. Read out of the
  // sensitivity report so the two can never disagree.
  const sensitivityDocument = JSON.parse(sensitivity) as {
    scenarios_attempted: number; scenarios_run: number; scenarios_not_runnable: unknown[];
  };
  writeMember(files, "scenarios/scenarios-not-run.json", canonicalJson({
    schema_version: "tivdoc-review-package-v6-scenarios",
    expected_by_brief: "42 scenario traces per branch",
    traces_included: 0,
    attempted: sensitivityDocument.scenarios_attempted,
    run: sensitivityDocument.scenarios_run,
    why_absent: "There are no traces to include. A draft RuleSpec cannot execute while its citation, rounding, effective-period, sector/population and precedence slots are unbound — the executor refuses it — and the 42 golden cases are blank templates with no input snapshot and no expected result. Every scenario is listed below with both reasons. An empty traces/ directory would have read as an oversight.",
    scenarios: sensitivityDocument.scenarios_not_runnable,
  }));

  const manifestContent = {
    schema_version: "tivdoc-review-package-v6-manifest",
    package: "review-package-v6",
    tenant: TENANT,
    ...STATE,
