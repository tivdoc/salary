...
const PACKAGE_ROOT = path.join("output", "next", "review-package-v12");
...
async function build(): Promise<{ manifest_sha256: string; files: Written[]; item_count: number }> {
  ...
  const sensitivity = readIfPresent("output/next/pool-q/decision-sensitivity-report-v7.json");
  if (!sensitivity) throw new Error("E48_SENSITIVITY_REPORT_V7_MISSING:run decision-sensitivity-run-v7.mts first");
  ...
  const factory = NodePostgresConnectionFactory.fromConnectionUrl({
    ...
    application_name: "tivdoc_review_package_v12",
    remote_dev_target: {
  ...
  try {
    await client.query(statement("v12_begin", "begin", []));
    await client.query(statement("v12_context", "select * from private.runtime_context_install($1,$2,$3)",
      [SYSTEM_SESSION.sid, SYSTEM_SESSION.jti, `v12:${randomUUID().slice(0, 8)}`]));
    const decisions = await client.query(statement("v12_decisions",
    ...
    for (const id of [...wanted].sort()) {
      ...
      const row = await client.query(statement("v12_aggregate",
    ...
    await client.query(statement("v12_rollback", "rollback", []));
  } finally {
    client.release();
  }

  writeMember(files, "decisions/legal-open-decisions.json", canonicalJson({
    schema_version: "tivdoc-review-package-v12-decisions",
  ...
  writeMember(files, "parameters/draft-parameters.json", canonicalJson({
    schema_version: "tivdoc-review-package-v12-parameters",
  ...
  writeMember(files, "rulespecs/templates.json", canonicalJson({
    schema_version: "tivdoc-review-package-v12-rulespec-templates",
    templates: buildSevenRuleSpecTemplates(),
  }));
  writeMember(files, "rulespecs/executable-sensitivity-specs.json", canonicalJson({
    schema_version: "tivdoc-review-package-v12-sensitivity-specs",
  ...
  writeMember(files, "rulespecs/drafts.json", canonicalJson({
    schema_version: "tivdoc-review-package-v12-rulespec-drafts",
  ...
  const previousManifest = readIfPresent("output/next/review-package-v11/manifest.json");
  const previousTopicsMember = readIfPresent("output/next/review-package-v11/scenarios/topics-not-run.json");
  ...
  writeMember(files, "scenarios/scenario-input-fixtures.json", canonicalJson({
    schema_version: "tivdoc-review-package-v12-scenario-fixtures",
  ...
  writeMember(files, "scenarios/executions-and-traces.json", canonicalJson({
    schema_version: "tivdoc-review-package-v12-executions",
  ...
  writeMember(files, "scenarios/topics-not-run.json", canonicalJson({
    schema_version: "tivdoc-review-package-v12-topics-not-run",
  ...
  writeMember(files, "shadow/branch-comparison.json", canonicalJson({
    schema_version: "tivdoc-review-package-v12-branch-comparison",
  ...
  writeMember(files, "shadow/synthetic-corpus-index.json", canonicalJson({
    schema_version: "tivdoc-review-package-v12-synthetic-corpus-index",
  ...
  writeMember(files, "visual-pages/index.json", canonicalJson({
    schema_version: "tivdoc-review-package-v12-visual-pages",
  ...
  writeMember(files, "parameters/citation-anchors.json", canonicalJson({
    schema_version: "tivdoc-review-package-v12-citation-anchors",
  ...
  const manifestContent = {
    schema_version: "tivdoc-review-package-v12-manifest",
    package: "review-package-v12",
    tenant: TENANT,
    ...STATE,
