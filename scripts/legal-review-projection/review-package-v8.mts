...
const PACKAGE_ROOT = path.join("output", "next", "review-package-v8");
...
async function build(): Promise<{ manifest_sha256: string; files: Written[]; item_count: number }> {
  ...
  const sensitivity = readIfPresent("output/next/pool-q/decision-sensitivity-report-v3.json");
  if (!sensitivity) throw new Error("E48_SENSITIVITY_REPORT_V3_MISSING:run decision-sensitivity-run-v3.mts first");
  ...
  const factory = NodePostgresConnectionFactory.fromConnectionUrl({
    ...
    application_name: "tivdoc_review_package_v8",
    remote_dev_target: {
  ...
  try {
    await client.query(statement("v8_begin", "begin", []));
    await client.query(statement("v8_context", "select * from private.runtime_context_install($1,$2,$3)",
      [SYSTEM_SESSION.sid, SYSTEM_SESSION.jti, `v8:${randomUUID().slice(0, 8)}`]));
    const decisions = await client.query(statement("v8_decisions",
    ...
    for (const id of [...wanted].sort()) {
      ...
      const row = await client.query(statement("v8_aggregate",
    ...
    await client.query(statement("v8_rollback", "rollback", []));
  } finally {
    client.release();
  }

  writeMember(files, "decisions/legal-open-decisions.json", canonicalJson({
    schema_version: "tivdoc-review-package-v8-decisions",
  ...
  writeMember(files, "parameters/draft-parameters.json", canonicalJson({
    schema_version: "tivdoc-review-package-v8-parameters",
  ...
  writeMember(files, "rulespecs/templates.json", canonicalJson({
    schema_version: "tivdoc-review-package-v8-rulespec-templates",
    templates: buildSevenRuleSpecTemplates(),
  }));
  writeMember(files, "rulespecs/executable-sensitivity-specs.json", canonicalJson({
    schema_version: "tivdoc-review-package-v8-sensitivity-specs",
  ...
  writeMember(files, "rulespecs/drafts.json", canonicalJson({
    schema_version: "tivdoc-review-package-v8-rulespec-drafts",
  ...
  // And one more gate for v8: a report running fewer topics than the last one
  // is a regression, and shipping it quietly is how a package stops meaning
  // anything. Four is what v3 reached; it may go up, never down.
  if (!(sensitivityDocument.topics_run.length >= 4)) {
    throw new Error(`E48_TOPICS_RUN_REGRESSED:got_${sensitivityDocument.topics_run.length}`);
  }
  writeMember(files, "scenarios/scenario-input-fixtures.json", canonicalJson({
    schema_version: "tivdoc-review-package-v8-scenario-fixtures",
  ...
  writeMember(files, "scenarios/executions-and-traces.json", canonicalJson({
    schema_version: "tivdoc-review-package-v8-executions",
  ...
  writeMember(files, "scenarios/topics-not-run.json", canonicalJson({
    schema_version: "tivdoc-review-package-v8-topics-not-run",
  ...
  writeMember(files, "parameters/citation-anchors.json", canonicalJson({
    schema_version: "tivdoc-review-package-v8-citation-anchors",
  ...
  const manifestContent = {
    schema_version: "tivdoc-review-package-v8-manifest",
    package: "review-package-v8",
    tenant: TENANT,
    ...STATE,
